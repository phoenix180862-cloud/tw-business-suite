        ...
        function StagingBereich({ kunde, onBack }) {
            ...
            if (view === 'detail' && activeBaustelle) {
                // Hotfix 20.04.2026: Callback-basierter Status-Reload
                // Ruft isStagingBereitgestellt erneut auf und aktualisiert die Prop,
                // die StagingDetail erhaelt. Damit verschwinden der orangene Banner
                // (fehlendeUnterordner) und ggf. der lila Banner (Migration) direkt
                // nach "Vervollstaendigen"/"Drive-Ordner umbenennen" ohne Zurueck-Klick.
                var statusReloadHandler = function() {
                    if (!window.TWStaging) return Promise.resolve();
                    return window.TWStaging.isStagingBereitgestellt(activeBaustelle.name)
                        .then(function(s) {
        ...
        // ═══════════════════════════════════════════════════════
        function StagingDetail({ baustelle, stagingStatus, onStatusReload, onBack, onOpenOrdner }) {
            ...
            async function handleStagingAnlegen() {
                ...
                try {
                    ...
                    // Hotfix 20.04.2026: Parent-Status (Prop) neu laden, damit
                    // der orangene "Einige Unterordner fehlen"-Banner verschwindet
                    if (typeof onStatusReload === 'function') {
            ...
            async function handleMigrationStarten() {
                ...
                try {
                    ...
                    await ladeInfo();
                    await pruefeMigration();
                    // Hotfix 20.04.2026: auch Parent-Status nachziehen
                    if (typeof onStatusReload === 'function') {
                        await onStatusReload();
                    }
                } catch(e) {
