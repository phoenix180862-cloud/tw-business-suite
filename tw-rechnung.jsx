        ...
        function RechnungsModul({ kunde, importResult, gesamtliste, aufmassGespeichert, onBack }) {
            ...
            var generatePDF=function(){
                ...
                var typLabel=rechnungsTyp==='abschlag'?(abschlagNr+'. Abschlagsrechnung'):rechnungsTyp==='schluss'?'Schlussrechnung':rechnungsTyp==='nachtrag'?('Nachtrag Nr. '+nachtragsNr):rechnungsTyp==='angebot'?('Angebot Nr. '+angebotsNr):'Rechnung';
                var posR='';aktivePosn.forEach(function(p){var gp=(parseFloat(p.menge)||0)*(parseFloat(p.einzelpreis)||0);posR+='<tr><td style="text-align:center;font-weight:700;width:30px">'+(p.pos||'')+'</td><td style="text-align:right;width:55px">'+fmt(p.menge)+'</td><td style="text-align:center;width:35px">'+(p.einheit||'')+'</td><td style="padding:6px 8px;word-break:break-word;white-space:pre-wrap;line-height:1.4">'+(p.bez||'').replace(/</g,'&lt;')+'</td><td style="text-align:right;width:65px">'+fmt(p.einzelpreis)+'</td><td style="text-align:right;width:75px;font-weight:700">'+fmt(gp)+'</td></tr>';});
                var h='<!DOCTYPE html><html><head><meta charset="utf-8"><title>'+getDateiname()+'</title>';
                ...
                h+='<style>@page{size:A4;margin:0}*{box-sizing:border-box;margin:0;padding:0}body{font-family:"Source Sans 3","Segoe UI",sans-serif;font-size:9.5pt;color:#222;line-height:1.4;background:#fff}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.page{page-break-after:always}.page:last-child{page-break-after:auto}table.p tr{page-break-inside:avoid}}';
                h+='.page{width:210mm;min-height:297mm;padding:18mm 18mm 30mm 22mm;margin:0 auto;position:relative;background:#fff;overflow:hidden}';
                h+='.logo-block{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4mm}.logo-inner{display:inline-flex;flex-direction:column;align-items:flex-start}.logo-thomas{font-family:"Source Sans 3",serif;font-style:italic;font-weight:700;color:#c41e1e;font-size:22px;margin-bottom:-24px;padding-left:1px;position:relative;z-index:2}.logo-word{display:flex;align-items:baseline;font-family:"Oswald",sans-serif;font-weight:700;color:#111;line-height:1}.logo-word .lw{font-size:81px}.logo-word .li-wrap{position:relative;font-size:81px;display:inline-block}.logo-word .li-char{font-size:81px;color:#111}.logo-word .li-dot{position:absolute;top:7px;left:50%;transform:translateX(-50%);width:13px;height:13px;background:#c41e1e}.logo-word .lLL{font-size:106px;letter-spacing:1px;line-height:0.75}.logo-word .lwa{font-size:81px}.logo-sub{display:flex;justify-content:flex-end;width:100%;margin-top:2px}.logo-sub span{font-family:"Source Sans 3",sans-serif;font-weight:600;color:#c41e1e;font-size:20px;letter-spacing:2.5px}.logo-right{text-align:right;font-size:9.5pt;color:#333;line-height:1.7;padding-top:5mm}';
                ...
                if(rechnungsTyp==='nachtrag')h+='<div><span class="dd-l">Nachtrags-Nr.:</span><span class="dd-v">'+nachtragsNr+'</span></div>';
                if(rechnungsTyp==='angebot')h+='<div><span class="dd-l">Angebots-Nr.:</span><span class="dd-v">'+angebotsNr+'</span></div>';
                ...
                if(rechnungsTyp==='angebot'&&gueltigBis)h+='<div class="r kl"><span>Gueltig bis:</span><span class="v">'+new Date(gueltigBis).toLocaleDateString('de-DE')+'</span></div>';
                h+='</div>';if(bemerkung)h+='<div class="bm">'+bemerkung.replace(/\n/g,'<br>')+'</div>';
                h+='<div class="ft"><div class="ft-row"><span class="ft-firma">Thomas Willwacher Fliesenlegermeister e.K.</span><span>Steuernummer: 30/220/1234/5</span></div><div class="ft-bank">Westerwald Bank eG \u00B7 IBAN: DE12 5739 1800 0000 0000 00 \u00B7 BIC: GENODE51WW1</div></div>';
                h+='</div></body></html>';
                var pw=window.open('','_blank','width=820,height=1160');pw.document.write(h);pw.document.close();setTimeout(function(){pw.focus();pw.print();},800);saveToGoogleDrive();
            };
            ...
            <div style={{margin:'8px',background:'#fff',borderRadius:'12px',padding:'22px 20px 16px',color:'#222',boxShadow:'0 4px 24px rgba(0,0,0,0.35)',minHeight:'85vh'}}>
                ...
                {/* --- BRIEFKOPF: Logo links + Adresse rechts (40% groesser als vorher) --- */}
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'3mm'}}>
                    <div style={{display:'inline-flex',flexDirection:'column',alignItems:'flex-start'}}>
                        <div style={{fontFamily:'"Source Sans 3",serif',fontStyle:'italic',fontWeight:700,color:'#c41e1e',fontSize:'18px',marginBottom:'-20px',paddingLeft:'1px',position:'relative',zIndex:2}}>Thomas</div>
                        <div style={{display:'flex',alignItems:'baseline',fontFamily:'Oswald,sans-serif',fontWeight:700,color:'#111',lineHeight:1}}>
