            ...
            const handleAkteSpeichern = async function() {
                ...
                try {
                    ...
                    wandMasse.forEach(function(w) {
                        raumRows.push({ Feld: 'Wand ' + w.id, Wert: w.wert || w.l || '' });
                    });
                    var ws1 = XLSX.utils.json_to_sheet(raumRows);
                    XLSX.utils.book_append_sheet(wb, ws1, 'Raumdaten');
                    // Blatt 2: Positionen (posCards direkt ohne calcPositionResult)
                    var posRows = posCards.map(function(p) {
                        return { Position: p.nr || p.pos, Bezeichnung: p.bezeichnung || p.bez, Einheit: p.einheit };
                    });
                    if (posRows.length > 0) {
