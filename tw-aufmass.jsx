        ...
        function Raumblatt({ kunde, raum, onFinishRaum, onBack, selectedPositions: initPositions, lastRaumData, gesamtliste, onShowGesamtliste, onAufmassBeenden }) {
            ...
            // ── Akten-Management (Skill: aufmass-details-erweiterung) ──
            ...
            // ── Wand-Verwaltung (dynamische Wandanzahl) ──
            const [wandAnzahl, setWandAnzahl] = useState(() => {
                if (reEdit && reEdit.wandAnzahl) return reEdit.wandAnzahl;
                if (reEdit && reEdit.wandMasse) return reEdit.wandMasse.length;
                if (hasData && raum.waende) return raum.waende.length;
                if (raum && raum.waende && raum.waende.length > 0) return raum.waende.length;
                return 4;
            });
            ...
            const [wandMasse, setWandMasse] = useState(() => {
                // RE-EDIT: gespeicherte Wandmasse uebernehmen
                if (reEdit && reEdit.wandMasse) return reEdit.wandMasse.map(function(w) { return {...w, l: w.l || w.wert || '', wert: w.wert || w.l || ''}; });
                // Aus erkanntem Raum
                if (hasData && raum.waende && raum.waende.length > 0) {
                    return raum.waende.map(function(w) { return { id: w.id, label: 'Wand ' + w.id, l: formatMass(w.l), wert: formatMass(w.l) }; });
                }
                // Manuell eingegebene Waende
                if (raum && raum.waende && raum.waende.length > 0) {
                    return raum.waende.map(function(w) { return { id: w.id, label: 'Wand ' + w.id, l: formatMass(w.l), wert: formatMass(w.l) }; });
                }
                // Default: 4 leere Waende
                var buchstaben = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
                var arr = [];
                for (var i = 0; i < 4; i++) {
                    arr.push({ id: buchstaben[i], label: 'Wand ' + buchstaben[i], l: '', wert: '' });
                }
                return arr;
            });

            // ── Handskizze erweitert ──
            const [zeichnenAktiv, setZeichnenAktiv] = useState(false); // DEFAULT: AUS!
            ...
            // ── Akten-Management Handler ──
            const handleAkteSpeichern = async function() {
                ...
                try {
                    ...
                    var exportData = {
                        ...
                        masse: masse,
                        wandMasse: wandMasse,
                        wandAnzahl: wandAnzahl,
                        tueren: tueren,
                        fenster: fenster,
                        abzuege: abzuege,
                        posCards: posCards.map(function(p) { return { nr: p.nr, bezeichnung: p.bezeichnung, einheit: p.einheit, ergebnis: calcPositionResult(p) }; }),
                        rbTab: rbTab,
                        datum: new Date().toISOString()
                    };
                    // Excel mit SheetJS erstellen
                    ...
                    wandMasse.forEach(function(w) {
                        raumRows.push({ Feld: 'Wand ' + w.id, Wert: w.wert || '' });
                    });
                    ...
                    var posRows = posCards.map(function(p) {
                        return { Position: p.nr, Bezeichnung: p.bezeichnung, Einheit: p.einheit, Ergebnis: calcPositionResult(p) };
                    });
                    if (posRows.length > 0) {
                    ...
                    if (kundeOrdnerId) {
                        // Unterordner Aufmass finden oder erstellen
                        ...
                        await service.uploadFile(blob, fileName, aufmassOrdner.id || aufmassOrdner);
                    } else {
            ...
            const loadBearbeiteteAkten = async function() {
                setAkteLoading(true);
                try {
                    var service = window.GoogleDriveService;
                    if (!service || !service.accessToken) {
                    ...
                    for (var oi = 0; oi < modulOrdner.length; oi++) {
                        ...
                        try {
                            var ordner = await service.findFolder(ordnerName, kundeOrdnerId);
                            if (ordner) {
                                ...
                                if (files && files.length > 0) {
                                    ergebnis.push({
                                        name: ordnerName,
                                        id: ordner.id || ordner,
                                        modul: ordnerName.toLowerCase(),
                                        files: files.map(function(f) {
                                            return { id: f.id, name: f.name, modifiedTime: f.modifiedTime, size: f.size };
                                        })
                                    });
                                }
                            }
                    ...
                    console.error('Akten laden fehlgeschlagen:', e);
                    setAkteOrdner([]);
                }
                setAkteLoading(false);
            };

            const handleAkteOeffnen = async function(ordner, file) {
                try {
                    var service = window.GoogleDriveService;
            ...
            // ── Wandanzahl aendern ──
            const handleWandAnzahlChange = function(neueAnzahl) {
                ...
                var neueWandMasse = [];
                for (var i = 0; i < n; i++) {
                    var existingWert = (wandMasse[i] && (wandMasse[i].wert || wandMasse[i].l)) || '';
                    neueWandMasse.push({
                        id: buchstaben[i],
                        label: 'Wand ' + buchstaben[i],
                        l: existingWert,
                        wert: existingWert
                    });
                }
            ...
            // ── Wandmass aendern mit Kopier-Logik ──
            const handleWandMassChange = function(idx, value) {
                var neueWandMasse = wandMasse.map(function(w) { return {...w}; });
                neueWandMasse[idx] = { ...neueWandMasse[idx], wert: value, l: value };
                // Masze-Kopier-Logik (NUR wenn aktiviert)
                if (masseKopieren) {
                    if (idx === 0 && neueWandMasse.length > 2) {
                        neueWandMasse[2] = { ...neueWandMasse[2], wert: value, l: value };
                    }
                    if (idx === 1 && neueWandMasse.length > 3) {
                        neueWandMasse[3] = { ...neueWandMasse[3], wert: value, l: value };
                    }
                }
                setWandMasse(neueWandMasse);
            };
            ...
            // ── Spracheingabe ──
            const startSpracheingabe = function() {
                var SR = window.SpeechRecognition || window.webkitSpeechRecognition;
                if (!SR) {
                    alert('Spracheingabe wird von diesem Browser nicht unterstuetzt.');
                    return;
                }
                ...
                recognition.onresult = function(event) {
                    ...
                    if (result.isFinal) {
                        ...
                        // Zahlen erkennen und in fokussiertes Eingabefeld eintragen
                        ...
                        if (zahlenMatch) {
                            ...
                            if (focusedInput && (focusedInput.classList.contains('masse-input') || focusedInput.tagName === 'INPUT')) {
                                ...
                                nativeInputValueSetter.call(focusedInput, zahlenMatch[1].replace(',', '.'));
                ...
                recognition.onerror = function(e) {
                    console.error('Spracheingabe Fehler:', e);
            ...
            const stopSpracheingabe = function() {
            ...
            return (
                <div className="page-container">
                    ...
                    {/* ═══ HAUPT-NAVIGATIONSLEISTE (8 Buttons, 2 Zeilen) ═══ */}
                    <div className="nav-toolbar-wrapper">
                        {/* Zeile 1: Standard-Navigation (6 Buttons) */}
                        <div className="nav-toolbar-row">
                            {onBack && (
                                <button className="nav-toolbar-btn" onClick={onBack}>Zurueck</button>
                            )}
                            {onShowGesamtliste && (
                                <button className="nav-toolbar-btn" onClick={function() { onShowGesamtliste(); }}>Gesamtliste</button>
                            )}
                            {onFinishRaum && (
                                <button className="nav-toolbar-btn" onClick={function() {
                                    handleTabChange(3);
                                }}>Raum fertig</button>
                            )}
                            {onAufmassBeenden && (
                                <button className="nav-toolbar-btn" onClick={onAufmassBeenden}>Aufmass beenden</button>
                            )}
                        </div>
                        {/* Zeile 2: Akten-Management (2 Buttons) */}
                        <div className="nav-toolbar-row">
                            <button className="nav-toolbar-btn akte-action"
                                onClick={function() { setAktePopupOpen(true); loadBearbeiteteAkten(); }}>
                                Bearbeitete Akte oeffnen
                            </button>
                            <button className="nav-toolbar-btn akte-action"
                                onClick={handleAkteSpeichern}>
                                {saveStatus === 'saving' ? 'Speichert...' : 'Akte speichern'}
                            </button>
                        </div>
                    </div>

                    {/* ═══ RAUMBLATT SUB-NAVIGATION (6 Buttons, 2 Zeilen) ═══ */}
                    <div className="raumblatt-sub-nav">
                        {/* Zeile 1: Tab-Buttons (Grundriss, Fotos, Oeffnungen, Positionen) */}
                        <div className="nav-toolbar-row">
                            {[
                                ...
                                var isActive = rbTab === tab.id;
                                return (
                                    <button key={tab.id}
                                        className={'nav-toolbar-btn' + (isActive ? ' active' : '')}
                                        onClick={function() { handleTabChange(tab.id); }}>
                                        {tab.label}
                                        {tab.id === 3 && posCards.length > 0 ? ' (' + posCards.filter(function(p) { return calcPositionResult(p) > 0; }).length + '/' + posCards.length + ')' : ''}
                                    </button>
                                );
                            })}
                        </div>
                        {/* Zeile 2: Wandanzahl + Masze kopieren */}
                        <div className="nav-toolbar-row">
                            <button className="nav-toolbar-btn"
                                onClick={function() { setWandAnzahlPopupOpen(true); }}>
                                Waende: {wandAnzahl}
                            </button>
                            <button className={'nav-toolbar-btn' + (masseKopieren ? ' active' : '')}
                                onClick={function() { setMasseKopieren(!masseKopieren); }}>
                                Masze kopieren {masseKopieren ? 'JA' : 'NEIN'}
                            </button>
                        </div>
                    </div>
                    ...
                    {/* ═══ POPUP: Bearbeitete Akte oeffnen ═══ */}
                    {aktePopupOpen && (
                        <div className="modal-overlay" onClick={function() { setAktePopupOpen(false); setAkteSelectedOrdner(null); }}>
                            <div className="modal-content" onClick={function(e) { e.stopPropagation(); }}
                                ...
                                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px'}}>
                                    <h3 style={{fontFamily:'Oswald', fontSize:'18px', fontWeight:700,
                                    ...
                                    <button style={{background:'none', border:'none', color:'var(--text-muted)',
                                        ...
                                        onClick={function() { setAktePopupOpen(false); setAkteSelectedOrdner(null); }}>
                                        ✕
                                    </button>
                                </div>
                                ...
                                {akteLoading ? (
                                    ...
                                    <div style={{padding:'40px', textAlign:'center', color:'var(--text-muted)'}}>
                                        Keine bearbeiteten Akten fuer diesen Kunden gefunden.
                                    </div>
                                ) : !akteSelectedOrdner ? (
                                    <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                                        {akteOrdner.map(function(ordner, i) {
                                            return (
                                                <button key={i} className="akte-popup-ordner"
                                                    onClick={function() { setAkteSelectedOrdner(ordner); }}>
                                                    <div>
                                                        <div style={{fontFamily:'Oswald', fontSize:'16px', fontWeight:600,
                                                            color:'var(--text-white)'}}>
                                                            {ordner.name}
                                                        </div>
                                                        <div style={{fontSize:'12px', color:'var(--text-muted)', marginTop:'4px'}}>
                                                            {ordner.files.length} Dokument{ordner.files.length !== 1 ? 'e' : ''}
                                                        </div>
                                                    </div>
                                    ...
                                    <div>
                                        <button style={{marginBottom:'12px', padding:'8px 16px', background:'var(--bg-tertiary)',
                                            border:'none', borderRadius:'8px', color:'var(--text-light)', cursor:'pointer',
                                            fontFamily:'Oswald', fontSize:'13px', fontWeight:600}}
                                            onClick={function() { setAkteSelectedOrdner(null); }}>
                                            &#8592; Zurueck
                                        </button>
                                        <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                                            {akteSelectedOrdner.files.map(function(file, i) {
                                                return (
                                                    <button key={i} className="akte-popup-file"
                                                        onClick={function() { handleAkteOeffnen(akteSelectedOrdner, file); }}>
                                                        <div style={{fontFamily:'Oswald', fontSize:'14px', fontWeight:600,
                                                            color:'var(--text-white)'}}>
                                                            {file.name}
                                                        </div>
                                                        <div style={{fontSize:'11px', color:'var(--text-muted)', marginTop:'4px'}}>
                                                            Zuletzt bearbeitet: {file.modifiedTime ? new Date(file.modifiedTime).toLocaleString('de-DE') : '--'}
                                                        </div>
                                                    </button>
                                                );
                                            })}
                    ...
                    {wandAnzahlPopupOpen && (
                        <div className="modal-overlay" onClick={function() { setWandAnzahlPopupOpen(false); }}>
                            <div className="modal-content" onClick={function(e) { e.stopPropagation(); }}
                                ...
                                <h3 style={{fontFamily:'Oswald', fontSize:'18px', fontWeight:700,
                                    textTransform:'uppercase', color:'var(--text-white)',
                                    marginBottom:'16px', textAlign:'center'}}>
                                    Anzahl der Waende
                                </h3>
                                <div className="masse-input-wrap" style={{marginBottom:'16px'}}>
                                    <input className="masse-input" type="number" min="3" max="12"
                                        value={wandAnzahl}
                                        onChange={function(e) { setWandAnzahl(parseInt(e.target.value) || 4); }}
                                        style={{textAlign:'center', fontSize:'24px', fontWeight:700}}
                                    />
                                </div>
                                <div style={{fontSize:'12px', color:'var(--text-muted)', textAlign:'center',
                                    marginBottom:'16px'}}>
                                    {wandAnzahl} Waende = Eingabefelder{' '}
                                    {'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.slice(0, Math.min(wandAnzahl, 26)).split('').join(', ')}
                                </div>
                                <button style={{width:'100%', padding:'12px', border:'none', borderRadius:'10px',
                                    background:'var(--accent-blue)', color:'white', fontFamily:'Oswald',
                            ...
                            <div className="modal-content" onClick={function(e) { e.stopPropagation(); }}
                                ...
                                <div style={{display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px'}}>
                                    ...
                                    <input type="range" min="1" max="10" value={stiftEmpfindlichkeit}
                                        ...
                                        className="empfindlichkeit-slider"
                                    />
                                    <span style={{color:'var(--text-muted)', fontSize:'12px'}}>Hoch</span>
                                </div>
                                <div style={{textAlign:'center', fontFamily:'Oswald', fontSize:'24px',
                                    fontWeight:700, color:'var(--accent-blue)', marginBottom:'16px'}}>
                                    {stiftEmpfindlichkeit}
                                </div>
                                <div style={{fontSize:'11px', color:'var(--text-muted)', textAlign:'center',
                                    marginBottom:'16px'}}>
                                    Niedrig = weniger Reaktion auf leichte Beruehrung.
                                    Hoch = reagiert auf jede Beruehrung.
                                </div>
                                <button style={{width:'100%', padding:'12px', border:'none', borderRadius:'10px',
                                    background:'var(--accent-blue)', color:'white', fontFamily:'Oswald',
                                    fontSize:'14px', fontWeight:700, cursor:'pointer'}}
                    ...
                    <div className="grundriss-card">
                        ...
                        {grundrissModus === 'skizze' && (
                            <React.Fragment>
                                ...
                                {/* ═══ NEUE HANDSKIZZE TOOLBAR (4 Buttons) ═══ */}
                                <div className="skizze-toolbar-new">
                                    <button className={'nav-toolbar-btn' + (zeichnenAktiv ? ' active' : '')}
                                        onClick={function() {
                                            setZeichnenAktiv(!zeichnenAktiv);
                                            if (!zeichnenAktiv) setSkizze(function(p) { return {...p, mode:'draw'}; });
                                        }}>
                                        {zeichnenAktiv ? 'Zeichnen AUS' : 'Zeichnen EIN'}
                                    </button>

                                    <div style={{position:'relative', flex:'1 1 45%'}}>
                                        <button className="nav-toolbar-btn"
                                        ...
                                        {loeschMenuOpen && (
                                            <div style={{
                                                ...
                                                <button className="nav-toolbar-btn" style={{width:'100%', marginBottom:'4px', minHeight:'40px', fontSize:'12px'}}
                                                ...
                                                <button className="nav-toolbar-btn" style={{width:'100%', minHeight:'40px', fontSize:'12px',
                                    ...
                                    <button className="nav-toolbar-btn"
                                        ...
                                        Empfindl. ({stiftEmpfindlichkeit})
                                    </button>

                                    <button className={'nav-toolbar-btn' + (spracheAktiv ? ' active' : '')}
                                        onClick={function() { spracheAktiv ? stopSpracheingabe() : startSpracheingabe(); }}>
                                        {spracheAktiv ? 'Sprache STOP' : 'Spracheingabe'}
                                    </button>
                                </div>

                                {/* Beschriften-Toggle (bleibt erhalten) */}
                                <div style={{display:'flex', gap:'4px', marginBottom:'8px'}}>
                                    <button className={'nav-toolbar-btn' + (skizze.mode === 'draw' ? ' active' : '')}
                                        style={{minHeight:'40px', fontSize:'12px'}}
                                        onClick={function() { setSkizze(function(p) { return {...p, mode:'draw'}; }); }}>
                                        Zeichnen
                                    </button>
                                    <button className={'nav-toolbar-btn' + (skizze.mode === 'label' ? ' active' : '')}
                                        style={{minHeight:'40px', fontSize:'12px', background: skizze.mode === 'label' ? '#e74c3c' : undefined}}
                                        onClick={function() { setSkizze(function(p) { return {...p, mode:'label'}; }); }}>
                                        Beschriften (A,B,C...)
                                    </button>
                                </div>
                    ...
                    <div className="raum-action-grid">
                        ...
                        <button className="raum-action-btn finish" onClick={() => handleFinishRaumValidated(() => {
                            ...
                            const raumState = {
                                masse: { ...masse }, wandMasse: wandMasse.map(w => ({...w})), wandAnzahl: wandAnzahl,
