        ...
        function Raumblatt({ kunde, raum, onFinishRaum, onBack, selectedPositions: initPositions, lastRaumData, gesamtliste, onShowGesamtliste, onAufmassBeenden }) {
            ...
            // ── KI-Grundriss: PDFs suchen und analysieren ──
            const startKiGrundriss = async () => {
                ...
                setKiGrundrissState(function(p) { return Object.assign({}, p, { status: 'suche-pdfs', statusText: 'Suche PDF-Zeichnungen im Kundenordner...', progress: 10 }); });
                ...
                try {
                    ...
                    var zeichnungsOrdner = (folderContents.folders || []).filter(function(f) {
                        var name = (f.name || '').toLowerCase();
                        return name.indexOf('03') >= 0 || name.indexOf('plan') >= 0 || name.indexOf('plaen') >= 0 ||
                               name.indexOf('zeichn') >= 0 || name.indexOf('grundriss') >= 0;
                    });

                    var alleFolders = zeichnungsOrdner.concat(
                        (folderContents.folders || []).filter(function(f) { return zeichnungsOrdner.indexOf(f) < 0; })
                    );

                    for (var fi = 0; fi < alleFolders.length; fi++) {
                        var folder = alleFolders[fi];
                        var subContents = folder.files ? folder : await driveService.listFolderContents(folder.id);
                        var pdfsInFolder = ((subContents.files || []).concat(subContents.pdfs || [])).filter(function(f) {
                            return (f.mimeType && f.mimeType.indexOf('pdf') >= 0) || (f.name && f.name.toLowerCase().indexOf('.pdf') >= 0);
                        });
                        pdfsInFolder.forEach(function(pdf) {
                            pdfFiles.push({ id: pdf.id, name: pdf.name, ordner: folder.name });
                        });
                    }
                    // Auch Root-PDFs
                    ((folderContents.files || []).concat(folderContents.pdfs || [])).forEach(function(f) {
                        if ((f.mimeType && f.mimeType.indexOf('pdf') >= 0) || (f.name && f.name.toLowerCase().indexOf('.pdf') >= 0)) {
                            pdfFiles.push({ id: f.id, name: f.name, ordner: 'Hauptordner' });
                        }
                    });

                    if (pdfFiles.length === 0) {
                        setKiGrundrissState(function(p) { return Object.assign({}, p, { status: 'fehler', statusText: 'Keine PDF-Zeichnungen gefunden', errorMsg: 'Im Kundenordner wurden keine PDF-Dateien gefunden.' }); });
                        return;
                    }

                    setKiGrundrissState(function(p) { return Object.assign({}, p, { pdfs: pdfFiles, status: 'analyse', statusText: pdfFiles.length + ' PDFs gefunden, starte KI-Analyse...', progress: 25 }); });
                    ...
                    for (var pi = 0; pi < pdfFiles.length; pi++) {
                        ...
                        setKiGrundrissState(function(p) { return Object.assign({}, p, {
                            ...
                            progress: 25 + Math.round((pi / pdfFiles.length) * 60)
                        }); });
                        ...
                        try {
                            ...
                            if (typeof window.callGeminiAPI === 'function') {
                                rawText = await window.callGeminiAPI(
                                    ...
                                    {
                                        model: proModelId,
                                        files: [{ mimeType: 'application/pdf', base64Data: pdfBase64 }],
                                        jsonMode: true
                                    }
                                );
                            }
                    ...
                    setKiGrundrissState(function(p) { return Object.assign({}, p, {
                        ...
                        errorMsg: 'Der Raum "' + (raumNr || '') + ' ' + (raumBez || '') + '" konnte in keiner der ' + pdfFiles.length + ' PDF(s) gefunden werden.',
                        progress: 100
                    }); });
            ...
            useEffect(function() {
                ...
                return function() { delete window._akteOeffnenHandler; delete window._akteSpeichernHandler; };
            }, []);
