        function Startseite({ onKundeNeu, onKundeAnalysiert, onKundeManuell, onKundenauswahl, onDriveStatusChange }) {
            ...
            var handleManualSync = async function() {
                ...
                if (!window.TWStorage || !window.TWStorage.isReady()) {
                    alert('Lokaler Speicher noch nicht bereit. Bitte kurz warten und erneut versuchen.');
                    return;
                }
                try {
                    ...
                    if (typeof alleAppDateien.then === 'function') alleAppDateien = await alleAppDateien;
                    var pendByKunde = {};
                    alleAppDateien.forEach(function(d) {
                        if (d.syncStatus === 'pending' && d.kundeId) {
                            pendByKunde[d.kundeId] = (pendByKunde[d.kundeId] || 0) + 1;
                        }
                    });
                    ...
                    var service = window.GoogleDriveService;
                    var rootFolderId = service && service._rootFolderId;
                    if (!rootFolderId && service && typeof service.findRootFolder === 'function') {
                        rootFolderId = await service.findRootFolder();
                    }
                    var ergebnisse = [];
                    for (var ki = 0; ki < kundenIds.length; ki++) {
                        ...
                        if (!kunde) {
                            ergebnisse.push({ kundeId: kid, status: 'skip', grund: 'Kunde nicht gefunden' });
                            continue;
                        }
                        var kundenFolderId = kunde.driveFolderId || kunde.folderId;
                        if (!kundenFolderId) {
                            ergebnisse.push({ kundeId: kid, name: kunde.name, status: 'skip', grund: 'Kein Drive-Folder' });
                            continue;
                        }

                        // Vor-Sync-Check: Wer hat zuletzt was geaendert?
                        var lastSync = await window.TWStorage.DriveUploadSync.getLastSyncTime(kid);
                        var driveCheck = await window.TWStorage.DriveUploadSync.checkDriveNewerThan(kundenFolderId, lastSync);
                        if (driveCheck.newer) {
                            var weiter = confirm(
                                '\u26A0\uFE0F Achtung bei Kunde "' + (kunde.name || kid) + '":\n\n' +
                                'Auf Google Drive sind ' + driveCheck.anzahlGeaenderter + ' Datei(en) NEUER als dein letzter Sync.\n' +
                                'Vermutlich hat ein Kollege parallel gearbeitet.\n\n' +
                                'Beispiel-Dateien:\n  - ' + (driveCheck.geaenderteDateien || []).slice(0, 3).join('\n  - ') +
                                '\n\nTrotzdem deine lokalen Aenderungen hochladen?\n' +
                                '(Drive-Versionen bleiben erhalten, nur deine eigenen Dateien werden hochgeladen.)'
                            );
                            if (!weiter) {
                                ergebnisse.push({ kundeId: kid, name: kunde.name, status: 'skip', grund: 'Konflikt - User hat abgebrochen' });
                                continue;
                            }
                        }
                        ...
                        // Upload starten
                        try {
                            var res = await window.TWStorage.DriveUploadSync.uploadAppDateien(kid, kundenFolderId, function(progress) {
                                console.log('[Sync ' + (kunde.name || kid) + '] ' + progress.message);
                            });
                            ergebnisse.push({ kundeId: kid, name: kunde.name, status: 'ok', uploaded: res.uploaded, total: res.total });
                        } catch(uploadErr) {
                            ergebnisse.push({ kundeId: kid, name: kunde.name, status: 'fehler', grund: uploadErr.message });
                        }
                    }
            ...
            return (
                <div className="startseite" style={{position:'relative', overflow:'hidden', minHeight:'100vh'}}>
                    ...
                    <div style={{padding:'0 20px', marginBottom:'10px', width:'100%', maxWidth:'500px'}}>
                        ...
                        {/* SYNC-BUTTON: Manueller Sync zu Google Drive */}
                        <div style={{textAlign:'center', marginTop:'10px'}}>
                            <button
                                onTouchEnd={function(e){ e.preventDefault(); handleManualSync(); }}
                                onClick={function(){ handleManualSync(); }}
                                disabled={!driveConnected}
                                style={{
                                    padding:'12px 22px', borderRadius:'var(--radius-md)',
                                    border:'1px solid rgba(46,204,113,0.45)',
                                    background: driveConnected ? 'linear-gradient(135deg, rgba(46,204,113,0.18), rgba(39,174,96,0.12))' : 'rgba(120,120,120,0.08)',
                                    color: driveConnected ? '#27ae60' : 'rgba(255,255,255,0.35)',
                                    cursor: driveConnected ? 'pointer' : 'not-allowed',
                                    fontSize:'13px', fontWeight:'700',
                                    fontFamily:'Oswald, sans-serif', textTransform:'uppercase', letterSpacing:'1px',
                                    transition:'all 0.2s ease',
                                    boxShadow: driveConnected ? '0 2px 8px rgba(46,204,113,0.15)' : 'none'
                                }}>
                                {'\uD83D\uDD04'} Jetzt mit Drive synchronisieren
                            </button>
                            {!driveConnected && (
                                <div style={{fontSize:'10px', color:'var(--text-muted)', marginTop:'4px', fontStyle:'italic'}}>
                                    Drive verbinden, um zu synchronisieren
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {/* ═══ BAUTEAM ANIMATION — Ivan & Michal laufen ueber den Bildschirm ═══ */}
                    <BauteamAnimation />

                    {/* ═══ Sonstige CSS Animations (Uhr-Pulse) ═══ */}
        ...
        function Kundenauswahl({ onSelect, loading, kunden, onUpdateKunde, kundeMode, onBack, onGoToModulwahl, onGoToDaten, onGoToOrdner }) {
            ...
            var toggleKundeForDelete = function(kundeKey) {
                setSelectedForDelete(function(prev) {
                    var n = Object.assign({}, prev);
                    if (n[kundeKey]) delete n[kundeKey];
                    else n[kundeKey] = true;
                    return n;
                });
            };

            var handleDeleteSelected = async function(kundenZuLoeschen) {
                var anzahl = kundenZuLoeschen.length;
                if (anzahl === 0) {
                    alert('Keine Kunden ausgewaehlt.');
                    return;
                }
                var liste = kundenZuLoeschen.map(function(k){ return '  - ' + k.name; }).join('\n');
                var bestaetigung = confirm(
                    '\u26A0\uFE0F LOESCHEN von ' + anzahl + ' Kunde(n) lokal:\n\n' + liste +
                    '\n\nDie Daten werden NUR lokal auf diesem Geraet geloescht.\n' +
                    'Auf Google Drive bleiben alle Daten unveraendert erhalten und koennen jederzeit neu geladen werden.\n\n' +
                    'Wirklich loeschen?'
                );
                if (!bestaetigung) return;
                ...
                setDeleting(true);
                var fehler = [];
                for (var i = 0; i < kundenZuLoeschen.length; i++) {
                    var k = kundenZuLoeschen[i];
                    var kundeId = k.id || k._driveFolderId || k.name;
                    try {
                        // 1. localStorage
                        var localKey = 'aufmass_kunde_' + kundeId;
                        localStorage.removeItem(localKey);
                        // Auch alternativer Schluessel mit kunde.name
                        if (k.name && k.name !== kundeId) {
                            localStorage.removeItem('aufmass_kunde_' + k.name);
                        }

                        // 2. IndexedDB
                        if (window.TWStorage && window.TWStorage.isReady() && window.TWStorage.deleteKundeData) {
                            await window.TWStorage.deleteKundeData(kundeId);
                        }
                    } catch(err) {
                        fehler.push({ name: k.name, error: err.message });
                    }
                }
                setDeleting(false);
                ...
                if (fehler.length > 0) {
                    alert('\u26A0\uFE0F ' + (anzahl - fehler.length) + '/' + anzahl + ' geloescht.\n\nFehler bei:\n' +
                        fehler.map(function(f){ return '  - ' + f.name + ': ' + f.error; }).join('\n'));
                } else {
                    alert('\u2705 ' + anzahl + ' Kunde(n) lokal geloescht.\n\nGoogle Drive bleibt unveraendert.\n\nDie Seite wird neu geladen.');
                }
                window.location.reload();
            };
            ...
            return (
                <div className="page-container">
                    ...
                    {dataSource.length === 0 && !searchTerm ? (
                        ...
                        <div>
                            {/* LOESCH-MODUS BANNER (oberhalb der Liste) */}
                                ...
                                {!deleteMode ? (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            fontFamily:'Oswald, sans-serif', textTransform:'uppercase', letterSpacing:'1px',
                                            boxShadow:'0 4px 12px rgba(196,30,30,0.3)', transition:'all 0.2s ease',
                                        }}
                                    >
                                        {'\uD83D\uDDD1\uFE0F'} Kunden-Daten loeschen
                                    </button>
                                ) : (
                                    <React.Fragment>
                                        <button
                                            onClick={function() {
                                                ...
                                                var zuLoeschen = filtered.filter(function(k) {
                                                    return selectedForDelete[k.id || k._driveFolderId || k.name];
                                                });
                                                handleDeleteSelected(zuLoeschen);
                                            }}
                                            disabled={Object.keys(selectedForDelete).length === 0 || deleting}
                                            style={{
                                                flex:1, padding:'12px 18px', borderRadius:'var(--radius-md)', border:'none',
                                                background: Object.keys(selectedForDelete).length > 0 && !deleting
                                                    ? 'linear-gradient(135deg, #c0392b, #922b21)' : 'rgba(120,120,120,0.15)',
                                                color: Object.keys(selectedForDelete).length > 0 && !deleting ? '#fff' : 'rgba(255,255,255,0.35)',
                                                cursor: Object.keys(selectedForDelete).length > 0 && !deleting ? 'pointer' : 'not-allowed',
                                                fontSize:'13px', fontWeight:'700',
                                                fontFamily:'Oswald, sans-serif', textTransform:'uppercase', letterSpacing:'1px',
                                                boxShadow: Object.keys(selectedForDelete).length > 0 && !deleting ? '0 4px 14px rgba(192,57,43,0.4)' : 'none',
                                                transition:'all 0.2s ease',
                                            }}
                                        >
                                            {deleting ? 'Loesche...' : ('\uD83D\uDDD1\uFE0F  ' + Object.keys(selectedForDelete).length + ' Kunde(n) loeschen')}
                                        </button>
                                        <button
                                            onClick={toggleDeleteMode}
                                            disabled={deleting}
                                            style={{
                                                padding:'12px 18px', borderRadius:'var(--radius-md)', border:'1px solid var(--border-color)',
                            ...
                            {deleteMode && (
                                <div style={{padding:'8px 12px', marginBottom:'10px', background:'rgba(192,57,43,0.10)', border:'1px solid rgba(192,57,43,0.3)', borderRadius:'8px', fontSize:'11px', color:'#e74c3c', textAlign:'center'}}>
                                    {'\u26A0\uFE0F'} Markiere die Kunden, deren lokale Daten geloescht werden sollen. Drive bleibt unangetastet.
                                </div>
                            )}

                            <div className="folder-list">
                            {filtered.map((kunde, idx) => {
                                var kundeKey = kunde.id || kunde._driveFolderId || kunde.name;
                                var isMarkedForDelete = !!selectedForDelete[kundeKey];
                                return (
                                <div
                                    ...
                                    style={{
                                        ...
                                        position:'relative',
                                        background: isMarkedForDelete ? 'rgba(192,57,43,0.12)' : undefined,
                                        border: isMarkedForDelete ? '1px solid rgba(192,57,43,0.4)' : undefined,
                                    }}
                                >
                                    {deleteMode && (
                                        <div
                                            onClick={function(){ toggleKundeForDelete(kundeKey); }}
                                            style={{
                                                width:'28px', height:'28px', borderRadius:'6px',
                                                border: '2px solid ' + (isMarkedForDelete ? '#e74c3c' : 'var(--border-color)'),
                                                background: isMarkedForDelete ? '#e74c3c' : 'transparent',
                                                cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center',
                                                color:'#fff', fontSize:'18px', fontWeight:'700', marginRight:'10px',
                                    ...
                                    <div
                                        ...
                                        onClick={function() {
                                            if (deleteMode) toggleKundeForDelete(kundeKey);
                                            else onSelect(kunde);
                                        }}
                                    >
        ...
        function Raumerkennung({ kunde, onSelectRaum, fertigeRaeume, lastRaumData, gesamtliste, onShowGesamtliste }) {
            ...
            // PAKET C: Listen-Bibliothek
            const [showSaveListe, setShowSaveListe] = useState(false);
            const [showLoadListe, setShowLoadListe] = useState(false);
            const [raeumeMitListe, setRaeumeMitListe] = useState({}); // { raumNr: true }
            const [gespListen, setGespListen] = useState([]); // [{id, bezeichner, raumIds, positionen, ...}]

            // Listen + Marker-Set bei Mount und nach jeder Aenderung neu laden
            const reloadListenStatus = React.useCallback(function() {
                if (!window.TWStorage || !window.TWStorage.isReady() || !kunde) return;
                var kid = kunde.id || kunde._driveFolderId || kunde._lvPositionenKey;
                if (!kid) return;
                window.TWStorage.loadPositionsListenByKunde(kid).then(function(items) {
                    setGespListen(items || []);
                    var raumSet = {};
                    (items || []).forEach(function(l) {
                        (l.raumIds || []).forEach(function(rid) { raumSet[rid] = true; });
                    });
                    setRaeumeMitListe(raumSet);
                }).catch(function(e) { console.warn('[Paket C] Listen laden:', e); });
            }, [kunde]);

            useEffect(function() { reloadListenStatus(); }, [reloadListenStatus]);
            ...
            const [raumAuswahlFrage, setRaumAuswahlFrage] = useState(null); // {raum, listen: [...]}
            ...
            const handleRaumClick = (raum) => {
                ...
                var raumKey = raum.nr || raum.id;
                // Gibt es eine vorbereitete Liste fuer diesen Raum?
                if (raeumeMitListe[raumKey]) {
                    var kid = kunde.id || kunde._driveFolderId || kunde._lvPositionenKey;
                    if (kid && window.TWStorage && window.TWStorage.loadPositionsListenByRaum) {
                        window.TWStorage.loadPositionsListenByRaum(kid, raumKey).then(function(listen) {
                            if (listen && listen.length > 0) {
                                setRaumAuswahlFrage({ raum: raum, listen: listen });
                                return;
                            }
                            openRaumMitStandard(raum);
                        }).catch(function() { openRaumMitStandard(raum); });
                        return;
                    }
                }
                openRaumMitStandard(raum);
            ...
            // Oeffnet einen Raum mit Standard-Vorauswahl (Vorraum-Positionen)
            const openRaumMitStandard = (raum) => {
            ...
            // Oeffnet einen Raum mit einer bestimmten vorbereiteten Liste
            const openRaumMitListe = (raum, liste) => {
                const preparedRaum = lastRaumData ? handleCopyToRecognized(raum) : raum;
                setActiveRaum(preparedRaum);
                setSelectedPositions(liste.positionen || []);
                setShowPosModal(true);
            };
                ...
                <div className="page-container">
                    ...
                    {Object.entries(grouped).map(([geschoss, raeume]) => (
                        <React.Fragment key={geschoss}>
                            ...
                            {raeume.map((raum, idx) => {
                                var hatListe = !!raeumeMitListe[raum.nr || raum.id];
                                return (
                                <div key={raum.nr} className={`raum-list-item ${isDone(raum.nr) ? 'done' : ''}`}
                                    ...
                                    <div className="raum-list-info">
                                        <div className="raum-list-bez">
                                            {isDone(raum.nr) && '✅ '}
                                            {hatListe && <span title="Vorbereitete Positionsliste verfügbar" style={{marginRight:'4px'}}>{'\uD83D\uDCCB'}</span>}
                                            {raum.bez}
                                        </div>
                    ...
                    {/* ══════════════════════════════════════════════════ */}
                    {/* PAKET C: LISTE SPEICHERN MODAL                     */}
                    {/* ══════════════════════════════════════════════════ */}
                    {showSaveListe && (
                        <div className="modal-overlay" onClick={() => setShowSaveListe(false)}>
                            <div className="modal" onClick={e => e.stopPropagation()} style={{maxWidth:'560px', maxHeight:'85vh', display:'flex', flexDirection:'column'}}>
                                <div className="modal-title" style={{display:'flex', alignItems:'center', gap:'10px'}}>
                                    <span style={{fontSize:'24px'}}>{'\uD83D\uDCBE'}</span>
                                    Positionsliste speichern
                                </div>
                                <div style={{fontSize:'13px', color:'var(--text-light)', marginBottom:'16px', lineHeight:'1.5'}}>
                                    Speichert die <strong>aktuelle Positionsauswahl ({selectedPositions.length} Positionen)</strong> als wiederverwendbare Liste.
                                    Du kannst sie einem oder mehreren Räumen zuordnen — oder als reine Vorlage ohne Raumbezug speichern.
                                </div>

                                {/* Bezeichner */}
                                <div className="manual-raum-field" style={{marginBottom:'12px'}}>
                                    <span className="manual-raum-label">Bezeichnung *</span>
                                    <input className="manual-raum-input" placeholder="z.B. Standard Bad, Bodenbelag Gäste-WC..."
                                        value={saveListBezeichner}
                                        onChange={e => setSaveListBezeichner(e.target.value)}
                                        autoFocus />
                                </div>

                                {/* Ohne-Raum-Haekchen */}
                                <label style={{display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', background:'rgba(230,126,34,0.08)', border:'1px solid rgba(230,126,34,0.2)', borderRadius:'var(--radius-sm)', marginBottom:'12px', cursor:'pointer'}}>
                                    <input type="checkbox" checked={saveListOhneRaum}
                                        onChange={e => setSaveListOhneRaum(e.target.checked)}
                                        style={{width:'18px', height:'18px', cursor:'pointer'}} />
                                    <span style={{fontSize:'13px', color:'var(--text-white)'}}>
                                        <strong>Als Vorlage speichern</strong> — keine Raumzuordnung (z.B. für zukünftige neue Räume)
                                    </span>
                                </label>

                                {/* Raum-Mehrfachauswahl */}
                                {!saveListOhneRaum && (
                                    <div style={{marginBottom:'16px', display:'flex', flexDirection:'column', minHeight:0, flex:1}}>
                                        <div className="manual-raum-label" style={{marginBottom:'6px'}}>
                                            Räume zuordnen (Mehrfachauswahl)
                                        </div>
                                        <div style={{
                                            border:'1px solid var(--border-color)', borderRadius:'var(--radius-sm)',
                                            ...
                                            {(kunde._masterRaeume || kunde.raeume || []).length === 0 && (
                                                <div style={{padding:'16px', textAlign:'center', color:'var(--text-muted)', fontSize:'12px'}}>
                                                    Keine Räume vorhanden. Aktiviere "Als Vorlage speichern" oben.
                                                </div>
                                            )}
                                            {(kunde._masterRaeume || kunde.raeume || []).map(function(r) {
                                                var rk = r.nr || r.id;
                                                var checked = !!saveListRaumIds[rk];
                                                return (
                                                    <label key={rk} style={{
                                                        display:'flex', alignItems:'center', gap:'10px',
                                                        padding:'6px 8px', borderRadius:'4px', cursor:'pointer',
                                                        background: checked ? 'rgba(39,174,96,0.12)' : 'transparent',
                                                        marginBottom:'2px'
                                                    }}>
                                                        <input type="checkbox" checked={checked}
                                                            onChange={e => {
                                                                var next = Object.assign({}, saveListRaumIds);
                                                                if (e.target.checked) next[rk] = true;
                                                                else delete next[rk];
                                                                setSaveListRaumIds(next);
                                                            }}
                                                            style={{width:'16px', height:'16px', cursor:'pointer'}} />
                                                        <span style={{fontSize:'13px', color:'var(--text-white)', flex:1}}>
                                                            <strong>{r.nr || '—'}</strong> {r.bez ? '· ' + r.bez : ''}
                                                            {r.geschoss ? <span style={{color:'var(--text-muted)', marginLeft:'6px'}}>({r.geschoss})</span> : null}
                                                        </span>
                                                        {raeumeMitListe[rk] && (
                                                            <span title="Hat bereits eine vorbereitete Liste" style={{fontSize:'14px'}}>{'\uD83D\uDCCB'}</span>
                                                        )}
                                                    </label>
                                                );
                                            })}
                                ...
                                {/* Aktionen */}
                                <div className="modal-btn-group" style={{marginTop:'auto'}}>
                                    <button className="modal-btn secondary" onClick={() => {
                                        setShowSaveListe(false);
                                        setSaveListBezeichner('');
                                        setSaveListRaumIds({});
                                        setSaveListOhneRaum(false);
                                    }}>Abbrechen</button>
                                    <button className="modal-btn primary"
                                        disabled={!saveListBezeichner.trim() || (!saveListOhneRaum && Object.keys(saveListRaumIds).length === 0)}
                                        style={{opacity: (!saveListBezeichner.trim() || (!saveListOhneRaum && Object.keys(saveListRaumIds).length === 0)) ? 0.4 : 1}}
                                        onClick={() => {
                                            var kid = kunde.id || kunde._driveFolderId || kunde._lvPositionenKey;
                                            var rids = saveListOhneRaum ? [] : Object.keys(saveListRaumIds);
                                            var payload = {
                                                kundeId: kid,
                                                bezeichner: saveListBezeichner.trim(),
                                                raumIds: rids,
                                                positionen: JSON.parse(JSON.stringify(selectedPositions || []))
                                            };
                                            window.TWStorage.savePositionsListe(payload).then(function() {
                                                setShowSaveListe(false);
                                                setSaveListBezeichner('');
                                                setSaveListRaumIds({});
                                                setSaveListOhneRaum(false);
                                                reloadListenStatus();
                                                alert('\u2705 Liste gespeichert — ' + (selectedPositions.length) + ' Positionen\n' +
                                                    (rids.length > 0 ? ('zugeordnet zu ' + rids.length + ' Raum/Räumen') : 'als Vorlage (ohne Raumbezug)'));
                                            }).catch(function(e) {
                    ...
                    {/* ══════════════════════════════════════════════════ */}
                    {/* PAKET C: LISTE LADEN / VERWALTEN MODAL             */}
                    {/* ══════════════════════════════════════════════════ */}
                    {showLoadListe && (
                        <div className="modal-overlay" onClick={() => setShowLoadListe(false)}>
                            <div className="modal" onClick={e => e.stopPropagation()} style={{maxWidth:'640px', maxHeight:'85vh', display:'flex', flexDirection:'column'}}>
                                <div className="modal-title" style={{display:'flex', alignItems:'center', gap:'10px'}}>
                                    <span style={{fontSize:'24px'}}>{'\uD83D\uDCC2'}</span>
                                    Gespeicherte Positionslisten
                                </div>
                                <div style={{fontSize:'13px', color:'var(--text-light)', marginBottom:'16px', lineHeight:'1.5'}}>
                                    {gespListen.length} {gespListen.length === 1 ? 'Liste' : 'Listen'} vorhanden. Eine Liste laden übernimmt die Positionen in die aktuelle Auswahl.
                                </div>
                                ...
                                <div style={{
                                    ...
                                    {gespListen.length === 0 && (
                                        <div style={{padding:'32px 16px', textAlign:'center', color:'var(--text-muted)', fontSize:'13px'}}>
                                            Keine gespeicherten Listen.<br/>
                                            <span style={{fontSize:'11px'}}>Positionen auswählen → "{'\uD83D\uDCBE'} Liste speichern" → dann kannst du sie hier wieder aufrufen.</span>
                                        </div>
                                    )}
                                    {gespListen.map(function(liste) {
                                        var raumBez = (liste.raumIds || []).length === 0
                                            ? [{nr:'—', bez:'Vorlage ohne Raumbindung', isVorlage:true}]
                                            : (liste.raumIds || []).map(function(rid) {
                                                var found = (kunde._masterRaeume || kunde.raeume || []).find(function(r){ return (r.nr || r.id) === rid; });
                                                return found ? found : {nr: rid, bez: '(nicht mehr vorhanden)'};
                                            });
                                        return (
                                            <div key={liste.id} style={{
                                                ...
                                                <div style={{display:'flex', alignItems:'flex-start', gap:'10px'}}>
                                                    <div style={{flex:1, minWidth:0}}>
                                                        <div style={{fontWeight:600, fontSize:'14px', color:'var(--text-white)', marginBottom:'4px'}}>
                                                            {liste.bezeichner || '(unbenannt)'}
                                                        </div>
                                                        <div style={{fontSize:'11px', color:'var(--text-muted)', marginBottom:'6px'}}>
                                                            {(liste.positionen || []).length} Positionen
                                                            {liste.updatedAt ? ' · ' + new Date(liste.updatedAt).toLocaleDateString('de-DE') : ''}
                                                        </div>
                                                        <div style={{display:'flex', flexWrap:'wrap', gap:'4px'}}>
                                                            {raumBez.map(function(r, i) {
                                                                return (
                                                                    <span key={i} style={{
                                                                        fontSize:'11px', padding:'2px 8px', borderRadius:'3px',
                                                                        background: r.isVorlage ? 'rgba(230,126,34,0.15)' : 'rgba(39,174,96,0.15)',
                                                                        color: r.isVorlage ? 'var(--accent-orange)' : 'var(--success)',
                                                                        fontWeight:500
                                                                    }}>
                                                                        {r.isVorlage ? r.bez : (r.nr + (r.bez ? ' · ' + r.bez : ''))}
                                                                    </span>
                                                                );
                                                            })}
                                                        </div>
                                                    ...
                                                    <div style={{display:'flex', flexDirection:'column', gap:'4px'}}>
                                                        <button className="modal-btn primary" style={{padding:'6px 12px', fontSize:'12px'}}
                                                            onClick={() => {
                                                                if (!activeRaum && !showPosModal) {
                                                                    // Wir sind auf der Raumerkennungsseite -> Liste nur als Zwischenspeicher, User muss erst Raum waehlen
                                                                    setSelectedPositions(JSON.parse(JSON.stringify(liste.positionen || [])));
                                                                    setShowLoadListe(false);
                                                                    alert('\u2705 ' + (liste.positionen||[]).length + ' Positionen geladen.\n\nJetzt Raum antippen, um mit diesen Positionen zu starten.');
                                                                } else {
                                                                    setSelectedPositions(JSON.parse(JSON.stringify(liste.positionen || [])));
                                                                    setShowLoadListe(false);
                                                                }
                                                        ...
                                                        <button className="modal-btn secondary" style={{padding:'6px 12px', fontSize:'12px', color:'var(--danger)'}}
                                                            onClick={() => {
                                                                if (!window.confirm('Liste "' + (liste.bezeichner||'') + '" endgültig löschen?')) return;
                                                                window.TWStorage.deletePositionsListe(liste.id).then(function() {
                                                                    reloadListenStatus();
                                                                });
                                                            }}>
                                                            {'\uD83D\uDDD1'} Löschen
                                ...
                                <div className="modal-btn-group" style={{marginTop:'12px'}}>
                                    <button className="modal-btn secondary" onClick={() => setShowLoadListe(false)}>Schließen</button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {/* ══════════════════════════════════════════════════ */}
                    {/* PAKET C: AUSWAHLFRAGE "Standard oder Vorbereitet?"  */}
                    {/* ══════════════════════════════════════════════════ */}
                    {raumAuswahlFrage && (
                        <div className="modal-overlay" onClick={() => setRaumAuswahlFrage(null)}>
                            <div className="modal" onClick={e => e.stopPropagation()} style={{maxWidth:'460px'}}>
                                <div className="modal-title" style={{display:'flex', alignItems:'center', gap:'10px'}}>
                                    <span style={{fontSize:'24px'}}>{'\uD83D\uDCCB'}</span>
                                    Raum {raumAuswahlFrage.raum.nr || ''} — Positionsliste wählen
                                </div>
                                <div style={{fontSize:'13px', color:'var(--text-light)', marginBottom:'16px', lineHeight:'1.5'}}>
                                    Für diesen Raum ist eine vorbereitete Liste gespeichert.
                                    Wie möchtest du starten?
                                </div>

                                <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
                                    {raumAuswahlFrage.listen.map(function(l) {
                                        return (
                                            <button key={l.id}
                                                className="modal-btn primary"
                                                style={{padding:'14px 16px', textAlign:'left', display:'flex', flexDirection:'column', alignItems:'flex-start', gap:'4px'}}
                                                onClick={() => {
                                                    var r = raumAuswahlFrage.raum;
                                                    setRaumAuswahlFrage(null);
                                                    openRaumMitListe(r, l);
                                                }}>
                                                <div style={{fontSize:'14px', fontWeight:600}}>
                                                    {'\uD83D\uDCCB'} {l.bezeichner || '(vorbereitete Liste)'}
                                                </div>
                                                <div style={{fontSize:'11px', opacity:0.85}}>
                                                    {(l.positionen||[]).length} Positionen · zuletzt geändert {l.updatedAt ? new Date(l.updatedAt).toLocaleDateString('de-DE') : ''}
                                                </div>
                                            </button>
                                        );
                                    })}
                                    <button className="modal-btn secondary" style={{padding:'14px 16px'}}
                                        onClick={() => {
                                            var r = raumAuswahlFrage.raum;
                                            setRaumAuswahlFrage(null);
                                            openRaumMitStandard(r);
                                        }}>
                                        {'\uD83D\uDCDD'} Standard-Liste aus Raumdaten verwenden
                                    </button>
                                    <button className="modal-btn secondary" style={{padding:'8px 16px', fontSize:'12px'}}
                                        onClick={() => setRaumAuswahlFrage(null)}>
                                        Abbrechen
                                    </button>
                                </div>
                            </div>
                    ...
                    {/* PAKET C: SPEICHER-DIALOG fuer Positionsliste */}
                    {showSaveListe && (
                        <div className="pos-modal-overlay" onClick={function(e){ if(e.target===e.currentTarget) setShowSaveListe(false); }}>
                            <div style={{background:'var(--bg-secondary)', borderRadius:'12px', padding:'16px', maxWidth:'500px', width:'92%', margin:'auto', maxHeight:'85vh', overflowY:'auto'}}>
                                <div style={{fontSize:'15px', fontWeight:'700', color:'var(--text-primary)', marginBottom:'12px', fontFamily:'Oswald, sans-serif', letterSpacing:'0.5px'}}>
                                    {'\uD83D\uDCBE'} Positionsliste speichern
                                </div>
                                <div style={{fontSize:'11px', color:'var(--text-muted)', marginBottom:'10px'}}>
                                    {selectedPositions.length} Position(en) werden gespeichert.
                                </div>
                                <label style={{display:'block', fontSize:'11px', color:'var(--text-muted)', marginBottom:'4px', fontWeight:'600'}}>Bezeichner *</label>
                                <input type="text" inputMode="text" value={saveListBezeichner}
                                    onChange={function(e){ setSaveListBezeichner(e.target.value); }}
                                    placeholder="z.B. Standard-Bad, Küche-XL, ..."
                                    style={{width:'100%', padding:'8px 10px', fontSize:'13px', borderRadius:'6px', border:'1px solid var(--border-color)', background:'var(--bg-tertiary)', color:'var(--text-primary)', marginBottom:'12px', boxSizing:'border-box'}} />

                                <label style={{display:'flex', alignItems:'center', gap:'8px', fontSize:'12px', color:'var(--text-primary)', cursor:'pointer', marginBottom:'10px'}}>
                                    <input type="checkbox" checked={saveListOhneRaum}
                                        onChange={function(e){ setSaveListOhneRaum(e.target.checked); if(e.target.checked) setSaveListRaumIds({}); }} />
                                    <span>Ohne Raumzuordnung speichern (als Vorlage)</span>
                                </label>

                                {!saveListOhneRaum && (
                                    <div style={{marginBottom:'12px'}}>
                                        <div style={{fontSize:'11px', color:'var(--text-muted)', marginBottom:'6px', fontWeight:'600'}}>Einen oder mehrere Räume zuordnen:</div>
                                        <div style={{maxHeight:'200px', overflowY:'auto', border:'1px solid var(--border-color)', borderRadius:'6px', padding:'6px'}}>
                                            {erkannteRaeume.length === 0 ? (
                                                <div style={{fontSize:'11px', color:'var(--text-muted)', padding:'8px', textAlign:'center'}}>Keine Räume vorhanden.</div>
                                            ) : erkannteRaeume.map(function(r) {
                                                var key = r.nr || r.id;
                                                var checked = !!saveListRaumIds[key];
                                                return (
                                                    <label key={key} style={{display:'flex', alignItems:'center', gap:'8px', padding:'5px 6px', fontSize:'12px', cursor:'pointer', borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
                                                        <input type="checkbox" checked={checked}
                                                            onChange={function(){
                                                                setSaveListRaumIds(function(prev){
                                                                    var n = Object.assign({}, prev);
                                                                    if (n[key]) delete n[key]; else n[key] = true;
                                                                    return n;
                                                                });
                                                            }} />
                                                        <span>{r.nr} &mdash; {r.bez}</span>
                                                    </label>
                                                );
                                            })}
                                        </div>
                                ...
                                <div style={{display:'flex', gap:'8px', marginTop:'14px'}}>
                                    <button style={{flex:1, padding:'10px 16px', borderRadius:'8px', border:'none', cursor:'pointer', background:'linear-gradient(135deg, var(--accent-red-light), var(--accent-red))', color:'#fff', fontSize:'12px', fontWeight:'700', fontFamily:'Oswald, sans-serif', textTransform:'uppercase', letterSpacing:'0.5px'}}
                                        onClick={function(){
                                            if (!saveListBezeichner.trim()) { alert('Bitte Bezeichner eingeben.'); return; }
                                            if (!saveListOhneRaum && Object.keys(saveListRaumIds).length === 0) {
                                                if (!confirm('Kein Raum ausgewählt. Als Vorlage ohne Raumzuordnung speichern?')) return;
                                            }
                                            var kid = kunde.id || kunde._driveFolderId || kunde._lvPositionenKey;
                                            window.TWStorage.savePositionsListe({
                                                kundeId: kid,
                                                bezeichner: saveListBezeichner.trim(),
                                                raumIds: saveListOhneRaum ? [] : Object.keys(saveListRaumIds),
                                                positionen: selectedPositions || []
                                            }).then(function() {
                                                alert('\u2705 Liste "' + saveListBezeichner + '" gespeichert.');
                                                setShowSaveListe(false);
                                                reloadListenStatus();
                                            }).catch(function(err){ alert('Fehler: ' + err.message); });
                                        }}>Speichern</button>
                                    <button style={{padding:'10px 16px', borderRadius:'8px', border:'1px solid var(--border-color)', cursor:'pointer', background:'transparent', color:'var(--text-secondary)', fontSize:'12px', fontWeight:'600'}}
                                        onClick={function(){ setShowSaveListe(false); }}>Abbrechen</button>
                                </div>
                            </div>
                        </div>
                    )}

                    {/* PAKET C: LADE-DIALOG fuer gespeicherte Positionslisten */}
                    {showLoadListe && (
                        <div className="pos-modal-overlay" onClick={function(e){ if(e.target===e.currentTarget) setShowLoadListe(false); }}>
                            <div style={{background:'var(--bg-secondary)', borderRadius:'12px', padding:'16px', maxWidth:'600px', width:'92%', margin:'auto', maxHeight:'85vh', overflowY:'auto'}}>
                                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'12px'}}>
                                    <div style={{fontSize:'15px', fontWeight:'700', color:'var(--text-primary)', fontFamily:'Oswald, sans-serif', letterSpacing:'0.5px'}}>
                                        {'\uD83D\uDCC2'} Gespeicherte Positionslisten
                                    </div>
                                    <button style={{padding:'4px 10px', borderRadius:'6px', border:'1px solid var(--border-color)', background:'transparent', color:'var(--text-secondary)', cursor:'pointer', fontSize:'11px'}}
                                        onClick={function(){ setShowLoadListe(false); }}>Schließen</button>
                                </div>
                                {gespListen.length === 0 ? (
                                    <div style={{padding:'24px', textAlign:'center', color:'var(--text-muted)', fontSize:'12px'}}>
                                        Noch keine Listen gespeichert. Nutze den Button "Liste speichern" um eine zu erstellen.
                                    </div>
                                ) : gespListen.map(function(l) {
                                    var raumNamen = (l.raumIds || []).map(function(rid) {
                                        var r = erkannteRaeume.find(function(er){ return (er.nr || er.id) === rid; });
                                        return r ? (r.nr + ' (' + r.bez + ')') : rid;
                                    }).join(', ');
                                    return (
                                        <div key={l.id} style={{border:'1px solid var(--border-color)', borderRadius:'8px', padding:'10px', marginBottom:'8px', background:'var(--bg-tertiary)'}}>
                                            <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'8px'}}>
                                                <div style={{flex:1, minWidth:0}}>
                                                    <div style={{fontSize:'13px', fontWeight:'700', color:'var(--text-primary)', marginBottom:'2px'}}>{l.bezeichner}</div>
                                                    <div style={{fontSize:'10px', color:'var(--text-muted)'}}>
                                                        {(l.positionen || []).length} Position(en) &middot; {raumNamen ? ('Räume: ' + raumNamen) : 'Vorlage (ohne Raum)'}
                                                    </div>
                                                    <div style={{fontSize:'9px', color:'var(--text-muted)', marginTop:'2px'}}>
                                                        {l.updatedAt ? new Date(l.updatedAt).toLocaleString('de-DE') : ''}
                                                    </div>
                                                </div>
                                                <div style={{display:'flex', gap:'4px', flexShrink:0}}>
                                                    <button style={{padding:'6px 10px', borderRadius:'6px', border:'none', background:'linear-gradient(135deg, #27ae60, #1e8449)', color:'#fff', cursor:'pointer', fontSize:'10px', fontWeight:'700'}}
                                                        onClick={function(){
                                                            setSelectedPositions(l.positionen || []);
                                                            setShowLoadListe(false);
                                                            alert('\u2705 ' + (l.positionen || []).length + ' Position(en) aus "' + l.bezeichner + '" geladen.');
                                                        }}>Laden</button>
                                                    <button style={{padding:'6px 10px', borderRadius:'6px', border:'1px solid rgba(231,76,60,0.4)', background:'rgba(231,76,60,0.1)', color:'#e74c3c', cursor:'pointer', fontSize:'10px'}}
                                                        onClick={function(){
                                                            if (!confirm('Liste "' + l.bezeichner + '" wirklich löschen?')) return;
                                                            window.TWStorage.deletePositionsListe(l.id).then(function(){
                                                                reloadListenStatus();
                                                            }).catch(function(err){ alert('Fehler: ' + err.message); });
                                                        }}>{'\uD83D\uDDD1\uFE0F'}</button>
                                                </div>
                    ...
                    {/* PAKET C: AUSWAHLFRAGE beim Raum-Klick (Standard vs. vorbereitet) */}
                    {raumAuswahlFrage && (
                        <div className="pos-modal-overlay" onClick={function(e){ if(e.target===e.currentTarget) setRaumAuswahlFrage(null); }}>
                            <div style={{background:'var(--bg-secondary)', borderRadius:'12px', padding:'20px', maxWidth:'480px', width:'92%', margin:'auto'}}>
                                <div style={{fontSize:'15px', fontWeight:'700', color:'var(--text-primary)', marginBottom:'8px', fontFamily:'Oswald, sans-serif'}}>
                                    {'\uD83D\uDCCB'} Welche Liste verwenden?
                                </div>
                                <div style={{fontSize:'12px', color:'var(--text-muted)', marginBottom:'16px'}}>
                                    Für <strong>{raumAuswahlFrage.raum.nr} ({raumAuswahlFrage.raum.bez})</strong> gibt es {raumAuswahlFrage.listen.length} vorbereitete Liste(n).
                                </div>
                                <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                                    <button style={{padding:'12px 16px', borderRadius:'8px', border:'none', cursor:'pointer', background:'linear-gradient(135deg, #1E88E5, #1565C0)', color:'#fff', fontSize:'13px', fontWeight:'700', fontFamily:'Oswald, sans-serif', letterSpacing:'0.5px'}}
                                        onClick={function(){
                                            var r = raumAuswahlFrage.raum;
                                            setRaumAuswahlFrage(null);
                                            openRaumMitStandard(r);
                                        }}>
                                        Standard-Vorauswahl nutzen
                                    </button>
                                    {raumAuswahlFrage.listen.map(function(l) {
                                        return (
                                            <button key={l.id} style={{padding:'12px 16px', borderRadius:'8px', border:'none', cursor:'pointer', background:'linear-gradient(135deg, #27ae60, #1e8449)', color:'#fff', fontSize:'13px', fontWeight:'700', fontFamily:'Oswald, sans-serif', letterSpacing:'0.5px', textAlign:'left'}}
                                                onClick={function(){
                                                    var r = raumAuswahlFrage.raum;
                                                    setRaumAuswahlFrage(null);
                                                    openRaumMitListe(r, l);
                                                }}>
                                                {'\uD83D\uDCCB'} {l.bezeichner} ({(l.positionen||[]).length} Pos.)
                                            </button>
                                        );
                                    })}
                                    <button style={{padding:'10px 16px', borderRadius:'8px', border:'1px solid var(--border-color)', cursor:'pointer', background:'transparent', color:'var(--text-secondary)', fontSize:'12px', fontWeight:'600'}}
                                        onClick={function(){ setRaumAuswahlFrage(null); }}>
                                        Abbrechen
                                    </button>
                                </div>
                            </div>
        ...
        function Raumblatt({ kunde, raum, onFinishRaum, onBack, selectedPositions: initPositions, lastRaumData, gesamtliste, onShowGesamtliste, onAufmassBeenden }) {
            ...
            // ── Helper: Alle Raum-Daten einsammeln (fuer Finish und Aufmass-Beenden) ──
            const buildRaumFinishPayload = () => {
            ...
            // ── Raumblatt fertigstellen (aktuellen Raum speichern, Rueckkehr zur Raumerkennung) ──
            const doRaumblattFertigstellen = () => {
            ...
            // ── Aufmass komplett fertigstellen (Raum speichern + Aufmass beenden + Weiter zum Rechnungsmodul) ──
            const doAufmassFertigstellen = () => {
                handleFinishRaumValidated(() => {
                    const payload = buildRaumFinishPayload();
                    onFinishRaum((raum && raum.nr), payload);
            ...
            // ── Zurueck mit Confirm-Dialog bei ungespeicherten Aenderungen ──
            const doZurueckZurRaumerkennung = () => {
            ...
            wipCollectorRef.current = function() {
                return {
                    moduleState: {
                        // Metadaten des aktiven Raums
                        activeRaum: raum,
                        activeSelectedPositions: initPositions || [],
                        activeRaumName: raumName,
                        activeTab: rbTab,
                        // Komplette Raumdaten
                        masse: masse,
                        wandMasse: wandMasse,
                        ...
                        posCards: posCards,
                        posRechenwegEdits: posRechenwegEdits,
                        fliesenUmlaufend: fliesenUmlaufend,
                        abdichtungUmlaufend: abdichtungUmlaufend,
                        fliesenDeckenhoch: fliesenDeckenhoch,
                        abdichtungDeckenhoch: abdichtungDeckenhoch,
                    ...
                    meta: {
                        beschreibung: 'Raumblatt: ' + (raumName || (raum && raum.bez) || (raum && raum.nr) || 'Raum'),
                        icon: '\uD83D\uDCD0',
                        raumNr: raum && raum.nr,
                        raumBez: raum && raum.bez
                    }
                };
            };
            ...
            React.useEffect(function() {
                ...
                var unsubCollect = window.TW.on('wip:collectState', function(data) {
                    if (!data || data.modulName !== 'aufmass') return;
                    if (wipCollectorRef.current && typeof data.callback === 'function') {
                        try { data.callback(wipCollectorRef.current()); }
                        catch(e) { console.error('[Raumblatt] wip:collectState Fehler:', e); }
                    }
                });
                var unsubRestore = window.TW.on('wip:restoreState', function(data) {
                    if (!data || data.modulName !== 'aufmass' || !data.moduleState) return;
                    var s = data.moduleState;
                    try {
                        if (s.activeRaumName !== undefined) setRaumName(s.activeRaumName);
                        if (typeof s.activeTab === 'number') setRbTab(s.activeTab);
                        if (s.masse) setMasse(s.masse);
                        if (s.wandMasse) setWandMasse(s.wandMasse);
                        if (s.tueren) setTueren(s.tueren);
                        if (s.fenster) setFenster(s.fenster);
                        if (s.abzuege) setAbzuege(s.abzuege);
                        if (s.posCards) setPosCards(s.posCards);
                        if (s.posRechenwegEdits) setPosRechenwegEdits(s.posRechenwegEdits);
                        if (typeof s.fliesenUmlaufend === 'boolean') setFliesenUmlaufend(s.fliesenUmlaufend);
                        if (typeof s.abdichtungUmlaufend === 'boolean') setAbdichtungUmlaufend(s.abdichtungUmlaufend);
                        if (typeof s.fliesenDeckenhoch === 'boolean') setFliesenDeckenhoch(s.fliesenDeckenhoch);
                        if (typeof s.abdichtungDeckenhoch === 'boolean') setAbdichtungDeckenhoch(s.abdichtungDeckenhoch);
                ...
                return function() {
                    if (typeof unsubCollect === 'function') unsubCollect();
