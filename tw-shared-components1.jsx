        ...
        function OrdnerBrowser({ kunde, onBack, onGoToDaten }) {
            ...
            var navigateToFolder = function(folder) {
                ...
                if (liveMode && folder.id && !folder._isAppOrdner) {
                    // Dateien aus dem Ordner-Objekt sofort anzeigen (listFolderContents packt files rein)
                    if (folder.files && folder.files.length > 0) {
                        setLiveFiles(folder.files);
                        setLiveFolders([]);  // Unterordner-Ordner werden erst beim Nachladen sichtbar
                    }
                    // Dann nochmal den Ordner-Inhalt von Drive laden fuer Unterordner
                    var svc = window.GoogleDriveService;
                    if (svc && svc.accessToken) {
                        setLoading(true);
