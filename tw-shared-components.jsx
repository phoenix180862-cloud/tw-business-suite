...
`}} />
        ...
        /* ═══════════════════════════════════════════
           KUNDEN-SYNC-BUTTON -- Pro-Kunde Drive-Synchronisation
           ───────────────────────────────────────────
           Sauberer Wrapper um TWStorage.DriveSync.syncKundenOrdner und
           TWStorage.DriveUploadSync.uploadAppDateien. Zeigt:
             - Phasen-State (scanning -> uploading -> downloading -> done)
             - Pending-Badge mit Anzahl ausstehender Uploads
             - Letzter Sync-Zeitpunkt
             - Inline-Fehler-Anzeige (KEIN alert/confirm-Spam)
           Verwendet ausschliesslich BESTEHENDE TWStorage-APIs. Kein neuer
           Speicher-Mechanismus, keine doppelten Pfade.
           ═══════════════════════════════════════════ */
        function KundenSyncButton({ kunde, onSyncComplete, compact }) {
            ...
            var kundeId = kunde && (kunde._driveFolderId || kunde.id || kunde.name);
            var driveFolderId = kunde && (kunde._driveFolderId || kunde.driveFolderId || kunde.folderId);

            // Pending-Count und letzten Sync-Zeitpunkt initial laden + alle 30s aktualisieren
            React.useEffect(function() {
                if (!kundeId || !window.TWStorage || !window.TWStorage.isReady()) return;

                var aktiv = true;
                var refresh = function() {
                    if (!aktiv) return;
                    // Pending-Count aus DriveUploadSync
                    if (window.TWStorage.DriveUploadSync && window.TWStorage.DriveUploadSync.hasUnsyncedChanges) {
                        window.TWStorage.DriveUploadSync.hasUnsyncedChanges(kundeId).then(function(res) {
                            if (aktiv && res) setPendingCount(res.count || 0);
                        }).catch(function() {});
                    }
                    // Letzter Sync aus DriveUploadSync
                    if (window.TWStorage.DriveUploadSync && window.TWStorage.DriveUploadSync.getLastSyncTime) {
                        window.TWStorage.DriveUploadSync.getLastSyncTime(kundeId).then(function(at) {
                            if (aktiv && at) setLastSyncAt(at);
                        }).catch(function() {});
                    }
                };
                refresh();
                var iv = setInterval(refresh, 30000);
                // Ticker fuer "vor X min" Anzeige
                var tick = setInterval(function() { forceTick(function(v) { return v + 1; }); }, 30000);
                return function() { aktiv = false; clearInterval(iv); clearInterval(tick); };
            }, [kundeId]);

            var handleSync = async function() {
                if (!kundeId || !driveFolderId || syncState.running) return;
                if (!window.TWStorage || !window.TWStorage.isReady()) {
                    setSyncState(function(p) { return Object.assign({}, p, { error: 'Lokaler Speicher noch nicht bereit.' }); });
                    return;
                }
                var service = window.GoogleDriveService;
                if (!service || !service.accessToken) {
                    setSyncState(function(p) { return Object.assign({}, p, { error: 'Google Drive nicht verbunden.' }); });
                    return;
                }

                setSyncState({ running: true, phase: 'scanning', progress: 0, total: 0, currentFile: 'Analysiere...', error: null, stats: null });

                try {
                    // PHASE 1: Lokal -> Drive (Upload pendender Dateien)
                    setSyncState(function(p) { return Object.assign({}, p, { phase: 'uploading', currentFile: 'Lokale Aenderungen werden hochgeladen...' }); });
                    var uploadRes = null;
                    if (window.TWStorage.DriveUploadSync && window.TWStorage.DriveUploadSync.uploadAppDateien) {
                        try {
                            uploadRes = await window.TWStorage.DriveUploadSync.uploadAppDateien(kundeId, driveFolderId, function(progress) {
                                setSyncState(function(p) {
                                    return Object.assign({}, p, {
                                        progress: progress.current || 0,
                                        total: progress.total || 0,
                                        currentFile: (progress.message || progress.fileName || '').substring(0, 80)
                                    });
                                });
                            });
                        } catch(uploadErr) {
                            // Upload-Fehler nicht abbrechen — Download-Phase trotzdem versuchen
                            console.warn('[KundenSync] Upload-Phase Fehler:', uploadErr);
                        }
                    }

                    // PHASE 2: Drive -> Lokal (bidirektionaler Sync inkl. Ordnerstruktur)
                    setSyncState(function(p) { return Object.assign({}, p, { phase: 'downloading', progress: 0, total: 0, currentFile: 'Drive wird analysiert...' }); });
                    var syncRes = await window.TWStorage.DriveSync.syncKundenOrdner(kundeId, driveFolderId, function(info) {
                        setSyncState(function(p) {
                            return Object.assign({}, p, {
                                phase: info.phase || 'downloading',
                                progress: (info.stats && (info.stats.dateienSynced + info.stats.dateienSkipped)) || 0,
                                total: (info.stats && info.stats.dateienGesamt) || 0,
                                currentFile: (info.message || '').substring(0, 80)
                            });
                        });
                    });
                    ...
                    setLastSyncAt(jetzt);
                    setPendingCount(0);
                    setSyncState({ running: false, phase: 'done', progress: 0, total: 0, currentFile: '', error: null, stats: syncRes && syncRes.stats });

                    if (onSyncComplete) onSyncComplete({ syncedAt: jetzt, uploadRes: uploadRes, syncRes: syncRes });
                    // Done-State nach 4 Sekunden zurueck auf idle
                    setTimeout(function() {
                        setSyncState(function(p) { return p.phase === 'done' ? Object.assign({}, p, { phase: 'idle' }) : p; });
                    }, 4000);
                } catch(err) {
                    console.error('[KundenSync] Fehler:', err);
                    setSyncState(function(p) { return Object.assign({}, p, { running: false, phase: 'error', error: err.message || 'Unbekannter Fehler' }); });
                }
            };

            // Label fuer aktuellen Phasen-State
            var labelText, iconChar, bgGradient, borderColor;
            if (syncState.running) {
                if (syncState.phase === 'scanning') { labelText = 'Analysiere...'; iconChar = '\u21BB'; }
                else if (syncState.phase === 'uploading') { labelText = 'Upload ' + (syncState.progress || 0) + (syncState.total ? '/' + syncState.total : ''); iconChar = '\u2191'; }
                else if (syncState.phase === 'downloading') { labelText = 'Download ' + (syncState.progress || 0) + (syncState.total ? '/' + syncState.total : ''); iconChar = '\u2193'; }
                else { labelText = 'Synchronisiere...'; iconChar = '\u21BB'; }
                bgGradient = 'linear-gradient(135deg, rgba(30,136,229,0.18), rgba(21,101,192,0.12))';
                borderColor = 'rgba(30,136,229,0.45)';
            } else if (syncState.phase === 'done') {
                labelText = 'Synchron!';
                iconChar = '\u2713';
                bgGradient = 'linear-gradient(135deg, rgba(46,204,113,0.18), rgba(39,174,96,0.12))';
                borderColor = 'rgba(46,204,113,0.45)';
            } else if (syncState.phase === 'error') {
                labelText = 'Sync-Fehler';
                iconChar = '!';
                bgGradient = 'linear-gradient(135deg, rgba(231,76,60,0.15), rgba(192,57,43,0.10))';
                borderColor = 'rgba(231,76,60,0.45)';
            } else {
                labelText = pendingCount > 0 ? 'Mit Drive synchronisieren' : 'Drive-Sync (alles aktuell)';
                iconChar = '\u2601';
                bgGradient = pendingCount > 0
                    ? 'linear-gradient(135deg, rgba(243,156,18,0.15), rgba(230,126,34,0.10))'
                    : 'linear-gradient(135deg, rgba(46,204,113,0.10), rgba(39,174,96,0.06))';
                borderColor = pendingCount > 0 ? 'rgba(243,156,18,0.45)' : 'rgba(46,204,113,0.30)';
            }

            // "vor X min" formatieren
            var sinceText = '';
            if (lastSyncAt) {
                var deltaMs = Date.now() - new Date(lastSyncAt).getTime();
                var mins = Math.floor(deltaMs / 60000);
                if (mins < 1) sinceText = 'gerade eben';
                else if (mins < 60) sinceText = 'vor ' + mins + ' Min.';
                else if (mins < 24 * 60) sinceText = 'vor ' + Math.floor(mins / 60) + ' Std.';
                else sinceText = 'vor ' + Math.floor(mins / 60 / 24) + ' Tagen';
            }

            var driveBereit = !!(driveFolderId && window.GoogleDriveService && window.GoogleDriveService.accessToken);

            return (
                <div style={{display:'flex', flexDirection:'column', gap:'6px', width:'100%'}}>
                    <button
                        onClick={handleSync}
                        disabled={syncState.running || !driveBereit}
                        style={{
                            display:'flex', alignItems:'center', gap:'10px',
                            width:'100%', padding: compact ? '10px 14px' : '12px 16px',
                            borderRadius:'var(--radius-md)',
                            border: '1px solid ' + borderColor,
                            background: driveBereit ? bgGradient : 'rgba(120,120,120,0.06)',
                            color:'var(--text-primary)',
                            fontFamily:'Oswald, sans-serif', fontSize: compact ? '12px' : '13px', fontWeight:700,
                            textTransform:'uppercase', letterSpacing:'0.5px',
                            cursor: (syncState.running || !driveBereit) ? 'not-allowed' : 'pointer',
                            transition:'all 0.2s ease',
                            opacity: driveBereit ? 1 : 0.55
                        }}>
                        <span style={{fontSize: compact ? '16px' : '18px', display:'inline-block', minWidth:'18px', textAlign:'center'}}>
                            {iconChar}
                        </span>
                        <span style={{flex:1, textAlign:'left'}}>{labelText}</span>
                        {pendingCount > 0 && !syncState.running && syncState.phase !== 'done' && (
                            <span style={{
                    ...
                    {/* Inline-Status: aktuelle Datei waehrend Sync */}
                    {syncState.running && syncState.currentFile && (
                        <div style={{
                            padding:'4px 12px',
                    ...
                    {/* Inline-Fehler */}
                    {syncState.error && !syncState.running && (
                        <div style={{
                    ...
                    {/* Letzter Sync Hinweis (idle + nicht-fehlerhaft) */}
                    {!syncState.running && syncState.phase !== 'error' && lastSyncAt && (
                        <div style={{
                            padding:'2px 12px',
                            fontSize:'10px', color:'var(--text-muted)',
                            fontFamily:'Source Sans 3, sans-serif'
                        }}>
                            Letzter Sync: {sinceText}
                        </div>
                    )}
                    ...
                    {!driveBereit && (
                        <div style={{
                            ...
                            {!driveFolderId ? 'Kein Drive-Ordner fuer diesen Kunden hinterlegt.' : 'Bitte zuerst Google Drive verbinden.'}
                        </div>
                    )}
                </div>
            );
