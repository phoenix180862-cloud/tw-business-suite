...
`}} />
        ...
        // ═══════════════════════════════════════════
        // Wiederhergestellte Komponenten aus index.html
        // (StatusPill, ToolbarButton, ToolbarRow, KundenSyncButton)
        // ═══════════════════════════════════════════
// Auto-extracted Components from index.html
// StatusPill, ToolbarButton, ToolbarRow, KundenSyncButton
// Babel pre-compiled (React.createElement form)

/* ---- StatusPill ---- */
function StatusPill({text,color,title,onClick}){// Farb-Schema: success (gruen), info (blau), warning (orange), danger (rot), muted (grau)
const colorMap={success:{bg:'rgba(46,160,67,0.18)',border:'rgba(46,160,67,0.45)',text:'#3fb950'},info:{bg:'rgba(30,136,229,0.18)',border:'rgba(30,136,229,0.45)',text:'#58a6ff'},warning:{bg:'rgba(243,156,18,0.18)',border:'rgba(243,156,18,0.45)',text:'#f0a84a'},danger:{bg:'rgba(196,30,30,0.18)',border:'rgba(196,30,30,0.45)',text:'#ff6b6b'},muted:{bg:'rgba(120,120,120,0.18)',border:'rgba(160,160,160,0.35)',text:'#c8c8c8'}};const c=colorMap[color]||colorMap.muted;return/*#__PURE__*/React.createElement("span",{title:title||'',onClick:onClick,style:{display:'inline-flex',alignItems:'center',padding:'4px 10px',borderRadius:'12px',background:c.bg,border:'1px solid '+c.border,color:c.text,fontSize:'11px',fontWeight:700,fontFamily:'Oswald, sans-serif',textTransform:'uppercase',letterSpacing:'0.6px',whiteSpace:'nowrap',cursor:onClick?'pointer':'default',userSelect:'none'}},text);}

/* ---- ToolbarButton ---- */
function ToolbarButton({icon,label,color,onClick,disabled,title}){// Farb-Schema fuer Buttons: red (default Aktion), blue (positive Aktion), orange (Auswahl), green (Bestaetigen)
const colorMap={red:{bg:'linear-gradient(135deg, #e84040, #c42323)',shadow:'rgba(196,30,30,0.35)'},blue:{bg:'linear-gradient(135deg, #4a90e2, #1e6fc4)',shadow:'rgba(30,136,229,0.35)'},orange:{bg:'linear-gradient(135deg, #f39c12, #d35400)',shadow:'rgba(211,84,0,0.35)'},green:{bg:'linear-gradient(135deg, #2ecc71, #1e8449)',shadow:'rgba(39,174,96,0.35)'},grey:{bg:'linear-gradient(135deg, #5d6d7e, #34495e)',shadow:'rgba(52,73,94,0.35)'}};const c=colorMap[color]||colorMap.red;return/*#__PURE__*/React.createElement("button",{onClick:disabled?undefined:onClick,title:title||label||'',disabled:!!disabled,style:{display:'inline-flex',alignItems:'center',gap:'5px',padding:'6px 11px',height:'32px',borderRadius:'8px',border:'none',background:disabled?'rgba(120,120,120,0.22)':c.bg,color:disabled?'rgba(255,255,255,0.4)':'#ffffff',fontSize:'12px',fontWeight:700,fontFamily:'Oswald, sans-serif',textTransform:'uppercase',letterSpacing:'0.6px',boxShadow:disabled?'none':'0 2px 8px '+c.shadow,cursor:disabled?'not-allowed':'pointer',whiteSpace:'nowrap',transition:'transform 0.15s ease, box-shadow 0.15s ease'}},icon&&/*#__PURE__*/React.createElement("span",{style:{fontSize:'14px',lineHeight:1}},icon),label&&/*#__PURE__*/React.createElement("span",null,label));}

/* ---- ToolbarRow ---- */
function ToolbarRow({left,right,sticky,stickyTop}){const stickyStyle=sticky===false?{}:{position:'sticky',top:(typeof stickyTop==='number'?stickyTop:60)+'px',zIndex:340};return/*#__PURE__*/React.createElement("div",{className:"tw-toolbar-row",style:{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'8px',padding:'6px 10px',background:'var(--bg-primary)',borderBottom:'1px solid var(--border-color)',flexWrap:'wrap',...stickyStyle}},/*#__PURE__*/React.createElement("div",{style:{display:'flex',alignItems:'center',gap:'6px',flexWrap:'wrap'}},left),/*#__PURE__*/React.createElement("div",{style:{display:'flex',alignItems:'center',gap:'6px',flexWrap:'wrap'}},right));}

/* ---- KundenSyncButton ---- */
function KundenSyncButton({kunde,onSyncComplete,compact}){const[syncState,setSyncState]=React.useState({running:false,phase:'idle',progress:0,total:0,currentFile:'',error:null,stats:null});const[pendingCount,setPendingCount]=React.useState(0);const[lastSyncAt,setLastSyncAt]=React.useState(null);const[,forceTick]=React.useState(0);var kundeId=kunde&&(kunde._driveFolderId||kunde.id||kunde.name);var driveFolderId=kunde&&(kunde._driveFolderId||kunde.driveFolderId||kunde.folderId);// Pending-Count und letzten Sync-Zeitpunkt initial laden + alle 30s aktualisieren
React.useEffect(function(){if(!kundeId||!window.TWStorage||!window.TWStorage.isReady())return;var aktiv=true;var refresh=function(){if(!aktiv)return;if(window.TWStorage.DriveUploadSync&&window.TWStorage.DriveUploadSync.hasUnsyncedChanges){window.TWStorage.DriveUploadSync.hasUnsyncedChanges(kundeId).then(function(res){if(aktiv&&res)setPendingCount(res.count||0);}).catch(function(){});}if(window.TWStorage.DriveUploadSync&&window.TWStorage.DriveUploadSync.getLastSyncTime){window.TWStorage.DriveUploadSync.getLastSyncTime(kundeId).then(function(at){if(aktiv&&at)setLastSyncAt(at);}).catch(function(){});}};refresh();var iv=setInterval(refresh,30000);var tick=setInterval(function(){forceTick(function(v){return v+1;});},30000);return function(){aktiv=false;clearInterval(iv);clearInterval(tick);};},[kundeId]);var handleSync=async function(){if(!kundeId||!driveFolderId||syncState.running)return;if(!window.TWStorage||!window.TWStorage.isReady()){setSyncState(function(p){return Object.assign({},p,{error:'Lokaler Speicher noch nicht bereit.'});});return;}var service=window.GoogleDriveService;if(!service||!service.accessToken){setSyncState(function(p){return Object.assign({},p,{error:'Google Drive nicht verbunden.'});});return;}setSyncState({running:true,phase:'scanning',progress:0,total:0,currentFile:'Analysiere...',error:null,stats:null});try{setSyncState(function(p){return Object.assign({},p,{phase:'uploading',currentFile:'Lokale Aenderungen werden hochgeladen...'});});var uploadRes=null;if(window.TWStorage.DriveUploadSync&&window.TWStorage.DriveUploadSync.uploadAppDateien){try{uploadRes=await window.TWStorage.DriveUploadSync.uploadAppDateien(kundeId,driveFolderId,function(progress){setSyncState(function(p){return Object.assign({},p,{progress:progress.current||0,total:progress.total||0,currentFile:(progress.message||progress.fileName||'').substring(0,80)});});});}catch(uploadErr){console.warn('[KundenSync] Upload-Phase Fehler:',uploadErr);}}setSyncState(function(p){return Object.assign({},p,{phase:'downloading',progress:0,total:0,currentFile:'Drive wird analysiert...'});});var syncRes=await window.TWStorage.DriveSync.syncKundenOrdner(kundeId,driveFolderId,function(info){setSyncState(function(p){return Object.assign({},p,{phase:info.phase||'downloading',progress:info.stats&&info.stats.dateienSynced+info.stats.dateienSkipped||0,total:info.stats&&info.stats.dateienGesamt||0,currentFile:(info.message||'').substring(0,80)});});});var jetzt=new Date().toISOString();setLastSyncAt(jetzt);setPendingCount(0);setSyncState({running:false,phase:'done',progress:0,total:0,currentFile:'',error:null,stats:syncRes&&syncRes.stats});if(onSyncComplete)onSyncComplete({syncedAt:jetzt,uploadRes:uploadRes,syncRes:syncRes});setTimeout(function(){setSyncState(function(p){return p.phase==='done'?Object.assign({},p,{phase:'idle'}):p;});},4000);}catch(err){console.error('[KundenSync] Fehler:',err);setSyncState(function(p){return Object.assign({},p,{running:false,phase:'error',error:err.message||'Unbekannter Fehler'});});}};var labelText,iconChar,bgGradient,borderColor;if(syncState.running){if(syncState.phase==='scanning'){labelText='Analysiere...';iconChar='\u21BB';}else if(syncState.phase==='uploading'){labelText='Upload '+(syncState.progress||0)+(syncState.total?'/'+syncState.total:'');iconChar='\u2191';}else if(syncState.phase==='downloading'){labelText='Download '+(syncState.progress||0)+(syncState.total?'/'+syncState.total:'');iconChar='\u2193';}else{labelText='Synchronisiere...';iconChar='\u21BB';}bgGradient='linear-gradient(135deg, rgba(30,136,229,0.18), rgba(21,101,192,0.12))';borderColor='rgba(30,136,229,0.45)';}else if(syncState.phase==='done'){labelText='Synchron!';iconChar='\u2713';bgGradient='linear-gradient(135deg, rgba(46,204,113,0.18), rgba(39,174,96,0.12))';borderColor='rgba(46,204,113,0.45)';}else if(syncState.phase==='error'){labelText='Sync-Fehler';iconChar='!';bgGradient='linear-gradient(135deg, rgba(231,76,60,0.15), rgba(192,57,43,0.10))';borderColor='rgba(231,76,60,0.45)';}else{labelText=pendingCount>0?'Mit Drive synchronisieren':'Drive-Sync (alles aktuell)';iconChar='\u2601';bgGradient=pendingCount>0?'linear-gradient(135deg, rgba(243,156,18,0.15), rgba(230,126,34,0.10))':'linear-gradient(135deg, rgba(46,204,113,0.10), rgba(39,174,96,0.06))';borderColor=pendingCount>0?'rgba(243,156,18,0.45)':'rgba(46,204,113,0.30)';}var sinceText='';if(lastSyncAt){var deltaMs=Date.now()-new Date(lastSyncAt).getTime();var mins=Math.floor(deltaMs/60000);if(mins<1)sinceText='gerade eben';else if(mins<60)sinceText='vor '+mins+' Min.';else if(mins<24*60)sinceText='vor '+Math.floor(mins/60)+' Std.';else sinceText='vor '+Math.floor(mins/60/24)+' Tagen';}var driveBereit=!!(driveFolderId&&window.GoogleDriveService&&window.GoogleDriveService.accessToken);return/*#__PURE__*/React.createElement("div",{style:{display:'flex',flexDirection:'column',gap:'6px',width:'100%'}},/*#__PURE__*/React.createElement("button",{onClick:handleSync,disabled:syncState.running||!driveBereit,style:{display:'flex',alignItems:'center',gap:'10px',width:'100%',padding:compact?'10px 14px':'12px 16px',borderRadius:'var(--radius-md)',border:'1px solid '+borderColor,background:driveBereit?bgGradient:'rgba(120,120,120,0.06)',color:'var(--text-primary)',fontFamily:'Oswald, sans-serif',fontSize:compact?'12px':'13px',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.5px',cursor:syncState.running||!driveBereit?'not-allowed':'pointer',transition:'all 0.2s ease',opacity:driveBereit?1:0.55}},/*#__PURE__*/React.createElement("span",{style:{fontSize:compact?'16px':'18px',display:'inline-block',minWidth:'18px',textAlign:'center'}},iconChar),/*#__PURE__*/React.createElement("span",{style:{flex:1,textAlign:'left'}},labelText),pendingCount>0&&!syncState.running&&syncState.phase!=='done'&&/*#__PURE__*/React.createElement("span",{style:{padding:'2px 8px',borderRadius:'10px',background:'rgba(243,156,18,0.20)',color:'#f39c12',fontSize:'11px',fontWeight:700,textTransform:'none',letterSpacing:0}},pendingCount," ausstehend")),syncState.running&&syncState.currentFile&&/*#__PURE__*/React.createElement("div",{style:{padding:'4px 12px',fontSize:'10px',color:'var(--text-muted)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:'Source Sans 3, sans-serif'}},syncState.currentFile),syncState.error&&!syncState.running&&/*#__PURE__*/React.createElement("div",{style:{padding:'8px 12px',borderRadius:'8px',background:'rgba(231,76,60,0.08)',border:'1px solid rgba(231,76,60,0.25)',fontSize:'11px',color:'#e74c3c',fontFamily:'Source Sans 3, sans-serif',lineHeight:'1.4'}},syncState.error),!syncState.running&&syncState.phase!=='error'&&lastSyncAt&&/*#__PURE__*/React.createElement("div",{style:{padding:'2px 12px',fontSize:'10px',color:'var(--text-muted)',fontFamily:'Source Sans 3, sans-serif'}},"Letzter Sync: ",sinceText),!driveBereit&&/*#__PURE__*/React.createElement("div",{style:{padding:'2px 12px',fontSize:'10px',color:'var(--text-muted)',fontStyle:'italic',fontFamily:'Source Sans 3, sans-serif'}},!driveFolderId?'Kein Drive-Ordner fuer diesen Kunden hinterlegt.':'Bitte zuerst Google Drive verbinden.'));}
