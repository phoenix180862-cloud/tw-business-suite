...
`}} />
        ...
        function AktionDropdown(props) {
            ...
            useEffect(function() {
                if (!open) return undefined;
                var handler = function(e) {
                    if (wrapperRef.current && !wrapperRef.current.contains(e.target)) { setOpen(false); setOpenSubId(null); }
                };
                var keyHandler = function(e) { if (e.key === 'Escape') { setOpen(false); setOpenSubId(null); } };
                document.addEventListener('mousedown', handler);
                document.addEventListener('touchstart', handler);
                document.addEventListener('keydown', keyHandler);
                return function() {
            ...
            var mainBtnStyle = { padding: '10px 18px', minHeight: '44px', borderRadius: '8px', border: 'none', cursor: 'pointer', background: palette.mainGrad, color: '#fff', fontSize: '13px', fontWeight: '700', fontFamily: 'Oswald, sans-serif', textTransform: 'uppercase', letterSpacing: '0.5px', boxShadow: '0 3px 10px ' + palette.mainShadow, transition: 'all 0.2s ease', whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: '8px' };
            var panelStyle = Object.assign({ position: 'absolute', top: 'calc(100% + 6px)', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '10px', boxShadow: '0 8px 16px rgba(0,0,0,0.4)', padding: '6px', minWidth: '220px', maxWidth: 'calc(100vw - 16px)', zIndex: 1000, animation: 'tw-nav-drop-in 140ms ease-out', display: 'flex', flexDirection: 'column', gap: '3px' }, align === 'left' ? { left: 0 } : { right: 0 });
            var subPanelStyle = { position: 'absolute', top: 0, right: 'calc(100% + 4px)', background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', borderRadius: '10px', boxShadow: '0 8px 16px rgba(0,0,0,0.45)', padding: '6px', minWidth: '200px', maxWidth: 'calc(100vw - 16px)', zIndex: 1001, display: 'flex', flexDirection: 'column', gap: '3px' };

            var itemStyle = function(disabled, destructive, variant) {
                var bg, col;
                if (disabled) { bg = 'rgba(120,120,120,0.15)'; col = 'rgba(255,255,255,0.4)'; }
                else if (destructive) { bg = 'transparent'; col = 'var(--accent-red-light)'; }
                else if (variant === 'red') { bg = 'linear-gradient(135deg, rgba(230,53,53,0.85), rgba(196,30,30,0.85))'; col = '#fff'; }
                else { bg = 'transparent'; col = 'var(--text-primary)'; }
                return { padding: '10px 12px', minHeight: '42px', borderRadius: '7px', border: 'none', cursor: disabled ? 'not-allowed' : 'pointer', background: bg, color: col, fontSize: '13px', fontWeight: variant === 'red' ? '700' : '600', fontFamily: 'Oswald, sans-serif', textTransform: variant === 'red' ? 'uppercase' : 'none', letterSpacing: variant === 'red' ? '0.4px' : '0', textAlign: 'left', transition: 'background 0.12s ease, filter 0.12s ease', display: 'flex', alignItems: 'center', gap: '10px', opacity: disabled ? 0.55 : 1 };
            };

            // Normalisiert beide Formate in ein einheitliches internes Format
            var normalizeAction = function(a) {
                if (!a) return null;
                if (a.type === 'divider') return { _divider: true };
                // Neues Format: type:'submenu' mit items:[{label,icon,onClick}]
                if (a.type === 'submenu') {
                    return {
                        id: a.id || a.label,
                        label: a.label,
                ...
                // Altes Format: subItems:[{id,label,icon,handler}]
                return {
                    id: a.id || a.label,
                    label: a.label,
                    icon: a.icon,
            ...
            var normalizedActions = actions.map(normalizeAction).filter(Boolean);

            var handleAction = function(a) {
                if (a.disabled) return;
                if (a._subItems && a._subItems.length > 0) {
                    setOpenSubId(function(cur) { return cur === a.id ? null : a.id; });
                    return;
                }
                setOpen(false); setOpenSubId(null);
                if (typeof a.handler === 'function') { try { a.handler(); } catch(err) { console.error('AktionDropdown handler:', err); } }
            };

            var handleSubItem = function(item) {
                if (item.disabled) return;
                setOpen(false); setOpenSubId(null);
                // Neues Format: onClick; Altes Format: handler
                var fn = item.onClick || item.handler;
                if (typeof fn === 'function') { try { fn(); } catch(err) { console.error('AktionDropdown sub:', err); } }
            };

            var hoverOn = function(e, variant) {
                if (variant === 'red') { e.currentTarget.style.filter = 'brightness(1.15)'; }
                else { e.currentTarget.style.background = 'var(--bg-hover)'; }
            };
            var hoverOff = function(e, variant) {
                if (variant === 'red') { e.currentTarget.style.filter = 'none'; }
            ...
            return (
                <div ref={wrapperRef} style={Object.assign({position:'relative', display:'inline-block'}, containerStyle)}>
                    ...
                    {open && normalizedActions.length > 0 && (
                        <div className={'tw-dropdown-panel ' + (align === 'right' ? 'tw-aktion-dropdown-panel-right' : 'tw-aktion-dropdown-panel-left')}
                            ...
                            {normalizedActions.map(function(a, idx) {
                                if (a._divider) {
                                    return <div key={'div-' + idx} style={{height:'1px', background:'var(--border-color)', margin:'3px 6px'}} />;
                                }
                                ...
                                return (
                                    <div key={a.id} style={{position:'relative'}}>
                                        <button type="button" role="menuitem" className="tw-aktion-dropdown-item"
                                            ...
                                            onMouseOver={function(e) { if (!a.disabled) hoverOn(e, a.variant); }}
                                            onMouseOut={function(e) { if (!a.disabled) hoverOff(e, a.variant); }}>
                                        ...
                                        {hasSub && isSubOpen && (
                                            <div className="tw-dropdown-panel tw-aktion-sub-panel" style={subPanelStyle} role="menu">
                                                {a._subItems.map(function(si) {
                                                    return (
                                                        <button key={si.id || si.label} type="button" role="menuitem"
                                                            className="tw-aktion-dropdown-item"
                                                            disabled={!!si.disabled}
                                                            style={itemStyle(si.disabled, si.destructive, si.variant)}
                                                            onClick={function() { handleSubItem(si); }}
                                                            onMouseOver={function(e) { if (!si.disabled) hoverOn(e, si.variant); }}
                                                            onMouseOut={function(e) { if (!si.disabled) hoverOff(e, si.variant); }}>
        ...
        /* CSS Keyframes + Dropdown-Animations + Mobile-Fixes */
        (function() {
            ...
            s.textContent =
                ...
                '   .tw-nav-dropdown-panel-center { left: 0 !important; transform: none !important; animation: tw-nav-drop-in-left 140ms ease-out !important; }' +
                '   .tw-aktion-dropdown-panel-right { right: 0 !important; left: auto !important; }' +
                '   .tw-aktion-sub-panel { right: auto !important; left: calc(100% + 4px) !important; }' +
                ' }';
            document.head.appendChild(s);
        })();
