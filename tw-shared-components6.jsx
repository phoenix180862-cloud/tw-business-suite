        ...
        function OrdnerBrowser({ kunde, onBack, onGoToDaten }) {
                ...
                async function load() {
                    ...
                    if (driveFolderId && service && service.accessToken) {
                        try {
                            ...
                            // Direkte API-Abfrage statt listFolderContents
                            var query = "'" + driveFolderId + "' in parents and trashed=false";
                            var data = await service._fetchJSON(
                                'https://www.googleapis.com/drive/v3/files?q=' + encodeURIComponent(query) +
                                '&fields=files(id,name,mimeType,size,modifiedTime)&orderBy=name&pageSize=200'
                            );
                            if (cancelled) return;
                            var items = data.files || [];
                            var initFolders = [], initFiles = [];
                            for (var ii = 0; ii < items.length; ii++) {
                                var itm = items[ii];
                                if (itm.mimeType === 'application/vnd.google-apps.folder') {
                                    initFolders.push({ id: itm.id, name: itm.name, files: [] });
                                } else {
                                    initFiles.push({
                                        id: itm.id, name: itm.name,
                                        type: service._getFileType ? service._getFileType(itm.mimeType, itm.name) : 'sonstige',
                                        size: itm.size ? (parseFloat(itm.size) / 1024 / 1024).toFixed(1) + ' MB' : '-',
                                        mimeType: itm.mimeType
                                    });
                                }
                            }
                            setLiveMode(true);
                            setLiveFolders(initFolders);
                            setLiveFiles(initFiles);
                            setLoading(false);
            ...
            var handleOpenFile = function(dateiId, fileName, isAppFile, liveDriveFileId) {
                // ═══ NEUES KONZEPT: Google Drive Direkt-Links ═══
                // Kein Download, kein Blob, kein iframe-Problem.
                // Oeffnet die Google-eigene Vorschau direkt im Browser.

                if (liveDriveFileId) {
                    // Google Drive Vorschau-URL oeffnen
                    var viewUrl = 'https://drive.google.com/file/d/' + liveDriveFileId + '/view';
                    window.open(viewUrl, '_blank');
                    return;
                }

                // Offline / App-Datei: Blob-URL in neuem Tab oeffnen
                if (isAppFile) {
            ...
            var navigateToFolder = function(folder) {
                ...
                if (liveMode && folder.id && !folder._isAppOrdner) {
                    // Sofort vorhandene Dateien zeigen
                    ...
                    // Direkte Drive API-Abfrage: Alle Kinder dieses Ordners laden
                    ...
                    if (svc && svc.accessToken) {
                        ...
                        svc._fetchJSON(
                            ...
                            for (var i = 0; i < items.length; i++) {
                                ...
                                if (item.mimeType === 'application/vnd.google-apps.folder') {
                                    folders.push({
                                        id: item.id,
                                        name: item.name,
                                        files: [] // Dateien werden beim Klick nachgeladen
                                    });
                                } else {
                                    var ft = svc._getFileType ? svc._getFileType(item.mimeType, item.name) : 'sonstige';
                                    files.push({
                                        id: item.id,
                                        name: item.name,
                                        type: ft,
                                        size: item.size ? (parseFloat(item.size) / 1024 / 1024).toFixed(1) + ' MB' : '-',
                            ...
                            setLiveFiles(files);
                        }).catch(function(e) { console.warn('Ordner laden:', e); });
                    }
                }
            };
                ...
                setBreadcrumb(function(prev) {
                    if (prev.length <= 1) {
                        ...
                        if (liveMode && driveFolderId) {
                            loadDriveFolderDirect(driveFolderId);
                        }
                        return [];
                    }
                    var nc = prev.slice(0, -1);
                    if (liveMode) {
                        // Im Live-Modus: Den Eltern-Ordner neu laden
                        loadDriveFolderDirect(nc[nc.length-1].id);
                    } else {
                        var t = findFolderInTree(tree, nc[nc.length-1].id);
                        setCurrentFolder(t);
                    }
                    return nc;
                });
            ...
            // Hilfsfunktion: Ordner-Inhalt direkt von Drive API laden
            function loadDriveFolderDirect(foldId) {
                var svc = window.GoogleDriveService;
                if (!svc || !svc.accessToken) return;
                var query = "'" + foldId + "' in parents and trashed=false";
                svc._fetchJSON(
                    'https://www.googleapis.com/drive/v3/files?q=' + encodeURIComponent(query) +
                    '&fields=files(id,name,mimeType,size,modifiedTime)&orderBy=name&pageSize=200'
                ).then(function(data) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        var item = items[i];
                        if (item.mimeType === 'application/vnd.google-apps.folder') {
                            folders.push({ id: item.id, name: item.name, files: [] });
                        } else {
                            files.push({
                                id: item.id, name: item.name,
                                type: svc._getFileType ? svc._getFileType(item.mimeType, item.name) : 'sonstige',
                                size: item.size ? (parseFloat(item.size) / 1024 / 1024).toFixed(1) + ' MB' : '-',
                                mimeType: item.mimeType
                            });
                        }
                    ...
                    setLiveFiles(files);
                    setLoading(false);
                }).catch(function(e) { console.warn('Drive laden:', e); setLoading(false); });
            }
